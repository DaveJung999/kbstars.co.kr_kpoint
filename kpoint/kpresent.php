...
//=======================================================
	...
	if (($_GET['mode'] ?? null) == 'p_modify' ){
		// [!] FIX: {$dbinfo['table_kpresent']}, {$_GET['uid']} 변수 중괄호 {} 적용
		$sql = "select * from {$dbinfo['table_kpresent']} 
		...
		if (($_GET['point'] ?? null) >= 0) $kpresent_list['point'] = $_GET['point'];
		if (!($kpresent_list['pdate'] ?? null)) $kpresent_list['pdate'] = date("Y-m-d");
		$kpresent_list['present'] = "KBSTARS양말";
	...
	if(($_GET['bid'] ?? null)){
		// [!] FIX: {$dbinfo['table_logon']}, {$_GET['bid']} 변수 중괄호 {} 적용
		$sql = "select * from {$dbinfo['table_logon']} 
				  where uid={$_GET['bid']} LIMIT 1";
		$klogon_list = db_arrayone($sql) or back('해당 데이터가 없습니다.');
		if(!($klogon_list['balance'] ?? null)) $klogon_list['balance'] = 0;
	
	}
	if(($_GET['s_id'] ?? null)){
		//시즌정보
		// [!] FIX: {$_GET['s_id']} 변수 중괄호 {} 적용
		$sql = " SELECT *
