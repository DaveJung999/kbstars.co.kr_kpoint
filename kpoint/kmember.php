...
//=======================================================
	...
	if (($_GET['mode'] ?? null) == 'modify' ){
		// [!] FIX: 테이블 및 $_GET 변수 중괄호 {} 적용
		$sql = "select a.uid, a.name, a.idnum, b.accountno, a.hp, b.comment , a.spts_cate
				...
				where a.uid = b.bid and a.uid={$_GET['bid']} and B.s_id = {$_GET['s_id']} LIMIT 1";
		$klogon = db_arrayone($sql) or back('해당 데이터가 없습니다.');
		
		$form_default = " method='post' action='ok.php'>";
		// [!] FIX: {$_GET}, {$klogon} 변수 중괄호 {} 적용
		$form_default .= substr(href_qs("mode=userinfomodify&bid={$klogon['uid']}&s_id={$_GET['s_id']}&cur_sid={$_GET['cur_sid']}&page={$_GET['page']}",'mode=',1),0,-1);
	}else{
		// 템플릿 마무리 할당
		$klogon = userGetDefaultFromTable($dbinfo['table']);
		
		if (isset($_GET['accountno'])) $klogon['accountno'] = $_GET['accountno'];
		
		userEnumSetFieldsToOptionTag($dbinfo['table'],$klogon); // $list['필드_option']에 enum,set필드 <option>..</option>생성
	
		$form_default = " method='post' action='ok.php'>";
		// [!] FIX: {$_GET} 변수 중괄호 {} 적용
		$form_default .= substr(href_qs("mode=userinfoadd&s_id={$_GET['s_id']}&cur_sid={$_GET['cur_sid']}&page={$_GET['page']}",'mode=',1),0,-1);
	}
...
// [!] FIX: {$_GET} 변수 중괄호 {} 적용
$href['klist']="klist.php?s_id={$_GET['s_id']}&cur_sid={$_GET['cur_sid']}&page={$_GET['page']}";
