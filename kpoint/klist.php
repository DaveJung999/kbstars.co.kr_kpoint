...
$sql = "SELECT A.uid, B.accountno, A.name, B.rdate, B.balance, A.spts_cate
		  FROM {$dbinfo['table_logon']} as A LEFT JOIN {$dbinfo['table_kpointinfo']} as B ON B.bid=A.uid
		  WHERE  $sql_where ORDER BY $sql_orderby, A.accountno  LIMIT $limitno,$limitrows";
...
while($list=db_array($rs_logon)) {
	...
	$href['inquiry'] = "kread.php?". href_qs('mode=inquiry&bid='.$list['uid']."&s_id=".$_GET['s_id']."&cur_sid=".$_GET['s_id']."&accountno=".$list['accountno'],$qs_basic);
	$href['member'] = "kmember.php?". href_qs('mode=modify&bid='.$list['uid']."&s_id=".$_GET['s_id']."&cur_sid=".$_GET['s_id']."&accountno=".$list['accountno'],$qs_basic);

/*	$sql_kpointinfo = "SELECT balance FROM {$dbinfo['table_kpointinfo']} WHERE bid={$list['uid']} and s_id = {$_GET['s_id']} ORDER BY uid";
...
$href['win']="kwin.php?".href_qs("mode=win&s_id={$_GET['s_id']}&act=&sdate=&edate=",$qs_basic);
$href['windelete']="kwin.php?".href_qs("mode=windelete&s_id={$_GET['s_id']}&act=&sdate=&edate=",$qs_basic);
$href['klist_excel']="klist_excel.php?".href_qs("mode=win&s_id={$_GET['s_id']}",$qs_basic);
$href['allreg']="kpoint.php?".href_qs("mode=allreg&s_id={$_GET['s_id']}",$qs_basic);
