...
// 기본 URL QueryString
$qs_basic = "db=".urlencode($db).					//table 이름
			"&mode=".					// mode값은 list.php에서는 당연히 빈값
			"&cateuid=".urlencode($cateuid).		//cateuid
			"&pern=".urlencode($pern) .	// 페이지당 표시될 게시물 수
			"&sc_column=".urlencode($sc_column).	//search column
			"&sc_string=" . urlencode(stripslashes($sc_string)) . //search string
			"&goto=".urlencode($goto) .	// 페이지당 표시될 게시물 수
			"&page=".urlencode($page);				//현재 페이지
...
$sql_where .= " AND `content` = 'familysite'	";
...
$count['total'] = $count_total_result['cnt'] ?? 0; // 전체 게시물 수
...
$tpl->set_file('html',"familysite.htm",1); // here 1 mean extract blocks
	...
	for($i=0; $i<$total; $i+=$dbinfo['row_pern']){
		...
		for($j=$i; ($j-$i < $dbinfo['row_pern']) && ($j < $total); $j++) { // 한줄에 여러값 출력시 루틴
			...
			if(in_array($file_ext, ['gif', 'jpg', 'jpeg', 'png', 'bmp'])) {
				$list['cur_banner'] = "<img src='" . htmlspecialchars($href['download'], ENT_QUOTES, 'UTF-8') . "' width='" . htmlspecialchars($list['data2'] ?? '', ENT_QUOTES, 'UTF-8') . "' height='" . htmlspecialchars($list['data3'] ?? '', ENT_QUOTES, 'UTF-8') . "' border='0'>";
			} elseif ($file_ext == 'swf') {
