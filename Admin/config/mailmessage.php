...
$uid = (int)($_REQUEST['uid'] ?? 1);

$thisPath	= dirname(__FILE__);
$thisUrl	= "/Admin/config"; // 마지막 "/"이 빠져야함
include_once("{$thisPath}/dbinfo.php"); // $dbinfo, $table_mail_message 값 정의

// 기본 URL QueryString
$qs_basic = "cateuid=".urlencode($cateuid).
			"&pern=".urlencode($pern) .
...
if(trim($sc_string)){
	...
	if($sc_column) {
		$sql_where .=" (`" . db_escape($sc_column) . "` LIKE '%{$sc_string_escaped}%') ";
	} else {
		$sql_where .=" ((`userid` LIKE '%{$sc_string_escaped}%') OR (`title` LIKE '%{$sc_string_escaped}%') OR (`content` LIKE '%{$sc_string_escaped}%')) ";
	}
}

if (empty($sql_where)) {
	$sql_where= " 1 and `uid` = {$uid} ";
}
...
//=====
// 페이지 나눔등 각종 카운트 구하기
$count_total_result = db_arrayone("SELECT count(*) as cnt FROM {$table_mail_message} WHERE  $sql_where ");
$count['total'] = $count_total_result['cnt'] ?? 0;
$count=board2Count($count['total'],$page,($dbinfo['pern'] ?? 10),($dbinfo['page_pern'] ?? 10)); // 각종 카운트 구하기
$count_today_result = db_arrayone("SELECT count(*) as cnt FROM {$table_mail_message} WHERE (rdate > unix_timestamp(curdate())) and $sql_where ");
...
$form_search =" name=search action='" . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . "' method='post'>
				<input type='hidden' name='db' value='" . htmlspecialchars($dbinfo['db'] ?? '', ENT_QUOTES, 'UTF-8') . "'>
...
if( !is_file("{$thisPath}/stpl/{$dbinfo['skin']}/mailmessage.htm") ) $dbinfo['skin']="basic";
$tpl->set_file('html',"{$thisPath}/stpl/{$dbinfo['skin']}/mailmessage.htm",TPL_BLOCK);

// Limit로 필요한 게시물만 읽음.
$sql = "SELECT * FROM {$table_mail_message} WHERE $sql_where group by `rdate` ORDER BY `rdate` DESC, `title` desc";
...
if(!$total=db_count($rs_list)) {	// 게시물이 하나도 없다면...
	...
	for($i=0; $i<$total; $i+=$dbinfo['row_pern']){
		...
		for($j=$i; ($j-$i < $dbinfo['row_pern']) && ($j < $total); $j++) { // 한줄에 여러값 출력시 루틴
			...
			$list['rdate_ymd']= isset($list['rdate']) ? date("Y/m/d H:i:s", $list['rdate']) : "";	//	날짜 변환
