...
$tpl->set_file('html',"singleline.htm",1); // here 1 mean extract blocks

// Limit로 필요한 게시물만 읽음.
$limitno = (int)($_GET['limitno'] ?? $count['firstno']);
$limitrows = (int)($_GET['limitrows'] ?? $count['pern']);
$rs_list = db_query("SELECT * FROM {$table} WHERE $sql_where ORDER BY `num`, `re` LIMIT {$limitno}, {$limitrows}");
