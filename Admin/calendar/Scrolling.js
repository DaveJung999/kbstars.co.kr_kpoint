	...
	this.scroll =	function () {
						...
						if ( !this.stop ) {
							for (i = 0; i < this.itemcount; i++) {
								...
								if ( this.type == 1 ) {
									obj.top = parseInt(obj.top) - this.scrollStepPixel();

//																	document.all.aaaa.value = obj.top;
	...
	this.scrollStepPixel =	function () {
								var toppix = parseInt(obj.top) % this.height;
								if(toppix < 0)
									toppix *= -1;
								return (toppix  > this.height - this.height / 10 ? 1 : parseInt(this.height / 10 / 2, 10) );
							};
}