...
// -------- ------ --------------------------------------
// 03/08/22 박선민 마지막 수정
// PHP 7 호환성 업데이트 (eregi_replace -> preg_replace, SQL WHERE 절 로직 수정)
//=======================================================
$HEADER=array(
...
// SQL문 where절 정리
//===================
$where_conditions = []; // WHERE 조건을 담을 배열
...
if(trim($act)){
	if(trim($sdate) && trim($edate)){
		...
		if($ssdate > $eedate) back("주문기간이 잘못설정했습니다");
		$where_conditions[] = " (rdate >= {$ssdate}) AND (rdate <= {$eedate}) ";
	}

	if(trim($name))	$where_conditions[] = " (name like '%{$name}%') ";
	if(trim($email))	$where_conditions[] = " (email like '%{$email}%') ";
	if(trim($c_name))	$where_conditions[] = " (c_name like '%{$c_name}%') ";
	if(trim($uid))		$where_conditions[] = " (userid like '%{$uid}%') ";
	if(trim($level))	$where_conditions[] = " (level like '%{$level}%') ";
	if(trim($tel))	$where_conditions[] = " (tel like '%{$tel}%') ";
	if(trim($search))	$where_conditions[] = " (region like '%{$search}%') ";
}

if(trim($sc_string)){
	if($sc_column)
		$where_conditions[] = " ({$sc_column} like '%{$sc_string}%') ";
	else
		$where_conditions[] = " ((userid like '%{$sc_string}%') or (title like '%{$sc_string}%') or (content like '%{$sc_string}%')) ";
}

$where_conditions[] = " (class='person' or	class='root' ) ";
$where_conditions[] = " {$table_logon}.uid = {$table_userinfo}.bid ";
...
//=====
// 페이지 나눔등 각종 카운트 구하기
$count['total']=db_result(db_query("SELECT count(*) FROM {$table_logon} WHERE  $sql_where "), 0, "count(*)"); // 전체 게시물 수
$count=board2Count($count['total'],$page,$dbinfo['pern'],$dbinfo['page_pern']); // 각종 카운트 구하기
$count['today']=db_result(db_query("SELECT count(*) FROM {$table_logon} WHERE (rdate > unix_timestamp(curdate())) and $sql_where ") , 0, "count(*)");

$limitno	= isset($_GET['limitno']) && $_GET['limitno'] ? $_GET['limitno'] : $count['firstno'];
$limitrows	= isset($_GET['limitrows']) && $_GET['limitrows'] ? $_GET['limitrows'] : $count['pern'];
...
// Limit로 필요한 게시물만 읽음.
$rs_list = db_query("SELECT * from {$table_logon} WHERE $sql_where ORDER BY c_name, userid LIMIT {$limitno},{$limitrows}");
if(!$total=db_count()) {	// 게시물이 하나도 없다면...
	...
	for($i=0; $i<$total; $i+=$dbinfo['row_pern']){
		...
		for($j=$i; ($j-$i < $dbinfo['row_pern']) && ($j < $total); $j++) { // 한줄에 여러값 출력시 루틴
			...
			// 포인트 조회, plus
			$balance=db_array(db_query(" SELECT sum(balance) as balance FROM new21_accountinfo where bid={$list['uid']} "));
...
$tpl->set_var("t_ch{$term_id}"			,${"t_ch{$term_id}"});// boardinfo 정보 변수
$tpl->set_var("term_id"			,$term_id);// boardinfo 정보 변수

${"s_{$search}"} = " selected";
$tpl->set_var("s_{$search}"			,${"s_{$search}"});// boardinfo 정보 변수
$tpl->set_var("search"			,$search);// boardinfo 정보 변수

$tpl->set_var('sdate'		,$sdate);	// form actions, hidden fileds
$tpl->set_var('edate'		,$edate);	// form actions, hidden fileds
