...
if (isset($userid)) {
	$where_conditions[] = " userid = '" . db_escape($userid) . "' ";
}

if (isset($_GET['priv']) && trim($_GET['priv'])){
	$where_conditions[] = " priv like '%" . db_escape($priv) . "%' ";
}
...
$tpl->set_file('html',"index_edit.htm",1); // here 1 mean extract blocks
...
$sql = "SELECT * from {$table_logon} WHERE $sql_where ORDER BY c_name, userid LIMIT {$_GET['limitno']},{$_GET['limitrows']}";
...
if(!$total=db_count()) {	// 게시물이 하나도 없다면...
	...
	for($i=0; $i<$total; $i+=$dbinfo['row_pern']){ // Added `?? 1` to prevent division by zero
		...
		for($j=$i; ($j-$i < $dbinfo['row_pern']) && ($j < $total); $j++) { // 한줄에 여러값 출력시 루틴
			...
			//우편번호 파싱
			$list['zip'] = explode("-",$list['zip']);
			$list['c_zip'] = explode("-",$list['c_zip']);

			//사업자번호
			$list['c_num'] = explode("-",$list['c_num']);
			$list['idnum_temp'] = explode("-",$list['idnum']);
			$list['idnum'] = $list['idnum_temp'][0]."-*******";
			$list['yesmail_c'][strval($list['yesmail'])] = " checked";
			$list['yessms_c'][strval($list['yessms'])] = " checked";
			$list['junior']= $list['p_name'] ? "1" : "";

			$list['birth_mm']	= substr($list['birth'],0,2);
			$list['birth_dd']	= substr($list['birth'],2,2);
			$list['birth_lunar']= substr($list['birth'],4,1);
			$list['birth_yyyy'] = substr($list['birth'],5,4);
			if(strlen($list['birth_yyyy']) == 4){
				$list['birth_date'] = $list['birth_yyyy'] . "-" . $list['birth_mm'] . "-" . $list['birth_dd'];
				if($list['birth_lunar'] == "-") $list['birth_lunar_c'][1]=" selected ";
				else $list['birth_lunar_c'][0]=" selected ";
			}
			$list['concern'] = explode(",",$list['concern']);
			foreach($list['concern'] as $v){
				$list['concern_c'][strval($v)] = " checked";
			}
			// 회원 구분 파싱
			$privs = explode(",",$list['priv']);
			foreach($privs as $key =>	$value){
				${"selected_priv_" . $value} = " selected ";
				$tpl->set_var("selected_priv_{$value}", ${"selected_priv_" . $value});
			}
...
$tpl->set_var("priv"			,isset($priv) ? $priv : '');//
// 블럭 : 첫페이지, 이전페이지
if($count['nowpage'] > 1){
