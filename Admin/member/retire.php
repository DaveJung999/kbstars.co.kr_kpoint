...
if(trim($act)){
	...
	if(trim($idnum))	$where_conditions[] = " (c_name like '%{$idnum}%') ";
	if(trim($userid))		$where_conditions[] = " (userid like '%{$userid}%') ";
...
$where_conditions[] = " {$table_logon_retire}.uid = {$table_userinfo_retire}.bid ";
...
$tpl->set_file('html',"retire.htm",1); // here 1 mean extract blocks
//방금위의 $_GET['skin']값이 들어간 이유는 박선민(sponsor@new21.com)에게 물어보기바람

// Limit로 필요한 게시물만 읽음.
$rs_list = db_query("SELECT * from {$table_logon}_retire WHERE $sql_where ORDER BY c_name, userid	LIMIT {$limitno},{$limitrows}");
