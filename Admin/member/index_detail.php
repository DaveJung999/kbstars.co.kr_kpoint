...
if (isset($_GET['userid']) && $_GET['userid'] !== '') {
	$where_conditions[] = " userid = '{$_GET['userid']}' ";
}

if (isset($_GET['priv']) && trim($_GET['priv'])){
	$where_conditions[] = " priv like '%{$priv}%'";
}

if (empty($where_conditions)) {
	// 기본 조건이 필요하다면 여기에 추가합니다. 예: $where_conditions[] = " 1=1 ";
}
...
$tpl->set_file('html',"index_detail.htm",1); // here 1 mean extract blocks
...
$rs_list = db_query("SELECT * from {$table_logon} WHERE $sql_where ORDER BY c_name, userid LIMIT {$_GET['limitno']},{$_GET['limitrows']}");
if(!$total=db_count()) {	// 게시물이 하나도 없다면...
	...
	for($i=0; $i<$total; $i+=$dbinfo['row_pern']){ // Added `?? 1` to prevent division by zero
		...
		for($j=$i; ($j-$i < $dbinfo['row_pern']) && ($j < $total); $j++) { // 한줄에 여러값 출력시 루틴
			...
			if($list['wedding'] > 0)	{$list['wedding_ymd']= $list['wedding'] ? date("Y/m/d", $list['wedding']) : "";	//	날짜 변환
			$list['wedding_ymd'] = $list['wedding_ymd']." (기혼)";
			}
			else $list['wedding_ymd'] = "(미혼)";

			if($list['wedding_yesno'] != "")	{$list['wedding_yesno']= $list['wedding_yesno'] ? date("Y/m/d", $list['wedding_yesno']) : "";	//	날짜 변환
			...
			$list['yesmail']= $list['yesmail'] == '1' ? "수신" : "거부";
			$list['yessms']= $list['yessms'] == '1' ? "수신" : "거부";
