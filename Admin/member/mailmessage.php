...
$thisUrl	= "/admin"; // 마지막 "/"이 빠져야함
	...
	$sql_where = " 1 AND " . implode(' AND ', $where_conditions);
	...
	//=====
	// 페이지 나눔등 각종 카운트 구하기
	$count['total']=db_result(db_query("SELECT count(*) FROM {$table_mail_message} WHERE  $sql_where "), 0, "count(*)"); // 전체 게시물 수
	$count=board2Count($count['total'],$page,$dbinfo['pern'],$dbinfo['page_pern']); // 각종 카운트 구하기
	$count['today']=db_result(db_query("SELECT count(*) FROM {$table_mail_message} WHERE (rdate > unix_timestamp(curdate())) and $sql_where ") , 0, "count(*)");
...
$tpl->set_file('html',"{$thisPath}/stpl/{$dbinfo['skin']}/mailmessage.htm", 1); // TPL_BLOCK 대신 1 사용

// Limit로 필요한 게시물만 읽음.
$rs_list = db_query("SELECT * from {$table_mail_message} WHERE $sql_where group by rdate ORDER BY rdate DESC, title desc");
if(!$total=db_count()) {	// 게시물이 하나도 없다면...
	...
	for($i=0; $i<$total; $i+=$dbinfo['row_pern']){
		...
		for($j=$i; ($j-$i < $dbinfo['row_pern']) && ($j < $total); $j++) { // 한줄에 여러값 출력시 루틴
			...
			$list['rdate_ymd']= $list['rdate'] ? date("Y/m/d H:i:s", $list['rdate']) : "";	//	날짜 변환
