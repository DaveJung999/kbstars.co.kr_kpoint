...
/*	if (document.ManageForm.category_code.value=='')
	...
	*/
	...
	document.ManageForm.action='ok.php?mode=modify_todaysbook&data2='+count;
...
function onHistwrite(){
	var rtn = confirm('위 도서를 오늘의 책 리스트로 저장하시겠습니까?');
	if (rtn == false) return;
	var LenOfWholemember,i;
	var QueryStr='';
	var count=0;
	...
	document.ManageForm.querystr.value=QueryStr;
	document.ManageForm.action='ok.php?mode=hist_write&data1=todaysbook&data2='+count;
	document.ManageForm.submit();
...
<form name='ManageForm' method=post onSubmit='javascript:return false;'>
	...
	<td>
		<table width="97%" border="0" align="center" cellpadding="0" cellspacing="0">
			...
			<td background="/images/admin/tbox_bg.gif"><strong>오늘의 책 관리 </strong></td>
		...
		<br>		<table width='97%' border='0' align="center" cellpadding='4' cellspacing='1' bgcolor='#aaaaaa'>
			<tr height=25 bgcolor=#F8F8EA>
			 <td height="25" bgcolor="#F0EBD6" align="center"><table width="98%"	border="0" align="center" cellpadding="0" cellspacing="0">
				<tr>
				 <td height="18" valign="middle"><img src="/images/admin/recmd_reserve.gif" width="16" height="15" border="0" align="absmiddle" /> <strong>오늘의 책 관리</strong></td>
				</tr>
			 </table></td>
			</tr>
			<tr bgcolor=#F8F8EA>
...
<?php
						$sql="Select * from {$SITE['th']}board2_books where $sql_where order by	data11, num desc, re";
						$rs_list = db_query($sql);
...
?>
				...
				<input name="show_all" type="button" class="stylebutton_blue" value=" 전체 " onClick="javascript:document.ManageForm.search_text.value='';javascript:document.ManageForm.submit();">
				</td>
			</tr>
		</table>
...
<?php
							$sql_hist = "select * from {$SITE['th']}board2_books_history where data1 = 'todaysbook' order by rdate desc";
