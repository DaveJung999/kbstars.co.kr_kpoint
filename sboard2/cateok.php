...
function cateSortOk($dbinfo,$qs){
	...
	$rs_srcuids=db_query("select * from {$dbinfo['table_cate']} where {$sql_where_cate} and num='".db_escape($src['num'])."' and re like '".db_escape($src['re'])."%'");
	$srcuids = array();
	...
	if(strlen($src['re'] ?? '')){
		if(($qs['dstuid'] ?? '') == 'first') { // 처음으로 이동한 경우 (re=ab라면 a보다 크고 ab보다 작은 범위를 1씩 증가후 re=a1으로 변경)
			...
			$sql = "select * from {$dbinfo['table_cate']} where {$sql_where_cate} and num='".db_escape($src['num'])."' and re like '".db_escape($re_prefix) . '_\' order by re LIMIT 1';
			$dst = db_arrayone($sql) or back('옮기고자 하는 카테고리 선택이 잘못되었습니다. err110');
		
			$src_length=strlen($src['re']);
			if(isset($dst['re']) && $dst['re'] == $re_prefix.'1' ) db_query("UPDATE {$dbinfo['table_cate']} SET re=concat( substring(re,1,{$src_length}-1), char(ord(substring(re,{$src['length']},1))+1 ), substring(re,{$src_length}+1) ) where {$sql_where_cate} and num='".db_escape($src['num'])."' and strcmp(re,'".db_escape($re_prefix)."')>0 and strcmp(re,'".db_escape($src['re'])."')< 0");
			db_query("UPDATE {$dbinfo['table_cate']} SET re=concat( substring(re,1,{$src_length}-1), '1', substring(re,{$src_length}+1)) where {$sql_where_srcuid_in}");
		} else {
			$dstuid_safe = db_escape($qs['dstuid']);
			$re_prefix = substr($src['re'],0,-1);
			$sql = "select * from {$dbinfo['table_cate']} where uid='{$dstuid_safe}' and num='".db_escape($src['num'])."' and re like '".db_escape($re_prefix) . '_\'';
			...
			if( strcmp($src['re'] ?? '',$dst['re'] ?? '') > 0 ){ // 상위로 이동할 경우 ( 목적위치+1이상에서 본래위치 미만 범위를 1씩 증가후 본래위치는 목적위치+1
				...
				db_query("UPDATE {$dbinfo['table_cate']} SET re=concat( substring(re,1,{$src_length}-1), char(ord(substring(re,{$src_length},1))+1 ), substring(re,{$src_length}+1) ) where {$sql_where_cate} and num='".db_escape($src['num'])."' and strcmp(re,'".db_escape($dst_re_next)."')>=0 and strcmp(re,'".db_escape($src['re'])."')< 0 ");
				db_query("UPDATE {$dbinfo['table_cate']} SET re=concat( substring(re,1,{$src_length}-1), right('".db_escape($dst_re_next)."',1), substring(re,{$src_length}+1)) where {$sql_where_srcuid_in}");
				...
				$dst_re_next=substr($dst['re'],0,-1) . chr(ord(substr($dst['re'],-1))+1);
				db_query("UPDATE {$dbinfo['table_cate']} SET re=concat( substring(re,1,{$src_length}-1), char(ord(substring(re,{$src_length},1))-1 ), substring(re,{$src_length}+1) ) where {$sql_where_cate} and num='".db_escape($src['num'])."' and strcmp(re,'".db_escape($src_re_next)."')>= 0 and strcmp(re,'".db_escape($dst_re_next)."')<0 ");
				db_query("UPDATE {$dbinfo['table_cate']} SET re=concat( substring(re,1,{$src_length}-1), right('".db_escape($dst['re'])."',1) , substring(re,{$src_length}+1) ) where {$sql_where_srcuid_in}");
			}
		}
		...
		if(($qs['dstuid'] ?? '') == 'first') { // 최상위로 이동할 경우
			...
			if(isset($dst['num']) && $dst['num'] == 1) db_query("UPDATE {$dbinfo['table_cate']} SET num=num+1 WHERE {$sql_where_cate} and num < ".db_escape($src['num']));
			...
			if(($src['num'] ?? 0) > ($dst['num'] ?? 0)){ // 상위로 이동할 경우 (dst[num]보다 크고 src[num] 미만범위를 1씩 증가후 src[num]=dst[num]+1로 변경
				db_query("UPDATE {$dbinfo['table_cate']} SET num=num+1 WHERE {$sql_where_cate} and num > ".db_escape($dst['num'])." and num < ".db_escape($src['num']));
				db_query("UPDATE {$dbinfo['table_cate']} SET num=".db_escape($dst['num'])."+1 WHERE {$sql_where_srcuid_in}");
			} elseif(($src['num'] ?? 0) < ($dst['num'] ?? 0)){ // 하위로 이동할 경우 (src[num]보다 크고 dst[num] 이하의 경우 1씩 감소후 본래위치는 dst[num]값으로
				db_query("UPDATE {$dbinfo['table_cate']} SET num=num-1 WHERE {$sql_where_cate} and num > ".db_escape($src['num'])." and num <= ".db_escape($dst['num']));
				db_query("UPDATE {$dbinfo['table_cate']} SET num=".db_escape($dst['num'])." WHERE {$sql_where_srcuid_in}");
			}
		}
	}
} // end func.
...
function cateDeleteOK($dbinfo,$field,$goto){
	...
	$sql="SELECT * FROM {$dbinfo['table_cate']} WHERE {$sql_where_cate} AND num={$list['num']} AND re LIKE '".db_escape($list['re']).'%\'';
	$rs2 = db_query($sql);
	if ($rs2) {
...
function getCateRe($table_cate, $sql_where_cate, $num, $re) { // 05/01/27 박선민
	...
	$sql="SELECT re, right(re,1) as last_char FROM {$table_cate} WHERE {$sql_where_cate} and num='".db_escape($num)."' AND length(re)=length('".db_escape($re)."')+1 AND locate('".db_escape($re)."', re)=1 ORDER BY re DESC LIMIT 1";
