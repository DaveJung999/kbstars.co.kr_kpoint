...
//=======================================================
	...
	$sql		= "SELECT * FROM {$dbinfo['table_cate']} WHERE uid='{$_GET['cateuid']}' and  {$sql_where_cate} ";
...
/////////////////////////
// $dstuid_options 구하기
if(isset($cateinfo['re']) && strlen($cateinfo['re']))
	$sql = "SELECT * FROM {$dbinfo['table_cate']} WHERE {$sql_where_cate} and num='{$cateinfo['num']}' and length(re) = length('{$cateinfo['re']}') and locate('" . substr($cateinfo['re'],0,-1) . "',re)=1 order by re";
else
...
$form_default .= href_qs("mode=catesort&db={$dbinfo['db']}&srcuid={$cateinfo['uid']}",'mode=',1);
