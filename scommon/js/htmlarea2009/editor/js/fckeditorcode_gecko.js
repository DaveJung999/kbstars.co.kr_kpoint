...
FCK.InitializeBehaviors = function() {
	...
	this._ExecDrop = function(evt) {
		...
		if (FCKConfig.ForcePasteAsPlainText) {
			if (evt.dataTransfer) {
				...
				FCK.InsertHtml(A);
			} else if (FCKConfig.ShowDropDialog) FCK.PasteAsPlainText();
			evt.preventDefault();
			evt.stopPropagation();
		}
	};
	this._ExecCheckCaret = function(evt) {
		...
		var E = function() {
			...
			if (H.nodeValue.length != G.endOffset) return;
			var I = H.parentNode.tagName.toLowerCase();
			if (!(I == 'a' || (!FCKBrowserInfo.IsOpera && String(H.parentNode.contentEditable) == 'false') || (!(FCKListsLib.BlockElements[I] || FCKListsLib.NonEmptyBlockElements[I]) && B == 35))) return;
			...
			if (J) {
				if (FCKBrowserInfo.IsOpera && B == 37) return;
				G.setStart(J, 0);
				G.setEnd(J, 0);
			} else {
				while (H.parentNode && H.parentNode != FCK.EditorDocument.body && H.parentNode != FCK.EditorDocument.documentElement && H == H.parentNode.lastChild && (!FCKListsLib.BlockElements[H.parentNode.tagName.toLowerCase()] && !FCKListsLib.NonEmptyBlockElements[H.parentNode.tagName.toLowerCase()])) H = H.parentNode;
				if (FCKListsLib.BlockElements[I] || FCKListsLib.EmptyElements[I] || H == FCK.EditorDocument.body) {
					...
					while (K) {
						...
						if (FCKListsLib.BlockElements[L] || FCKListsLib.EmptyElements[L] || FCKListsLib.NonEmptyBlockElements[L]) break;
						K = K.nextSibling;
					};
					var M = FCK.EditorDocument.createTextNode('');
					if (K) H.parentNode.insertBefore(M, K);
					else H.parentNode.appendChild(M);
					G.setStart(M, 0);
					G.setEnd(M, 0);
				}
			};
	...
	if (FCKBrowserInfo.IsGecko) {
		...
		this.EditorDocument.addEventListener('dragover', function(evt) {
			if (!FCK.MouseDownFlag && FCK.Config.ForcePasteAsPlainText) evt.returnValue = false;
		}, true);
		...
		this.EditorDocument.addEventListener('mouseup', function(ev) {
			if (ev.srcElement.nodeName.IEquals('INPUT', 'TEXTAREA', 'SELECT')) ev.preventDefault()
		}, true);
...
function Document_OnContextMenu(e) {
	if (!e.target._FCKShowContextMenu) e.preventDefault();
};
...
FCK.ExecuteRedirectedNamedCommand = function(A, B) {
	switch (A) {
		...
		case 'Paste':
			try {
				if (FCKBrowserInfo.IsSafari) throw '';
				if (FCK.Paste()) FCK.ExecuteNamedCommand('Paste', null, true);
			} catch (e) {
				if (FCKConfig.ForcePasteAsPlainText) FCK.PasteAsPlainText();
				else FCKDialog.OpenDialog('FCKDialog_Paste', FCKLang.Paste, 'dialog/fck_paste.html', 400, 330, 'Security');
			};
			break;
		default:
...
FCK.InsertHtml = function(A) {
	...
	if (FCKBrowserInfo.IsGecko) {
		...
		var E = C.RootNode.firstChild;
		while (E && E.nodeType != 1) E = E.nextSibling;
		if (E && FCKListsLib.BlockElements[E.nodeName.toLowerCase()]) range.SplitBlock();
		range.DeleteContents();
		range.InsertNode(C.RootNode);
		range.MoveToPosition(D, 4);
	} else B.execCommand('inserthtml', false, A);
	this.Focus();
	if (!range) {
		range = new FCKDomRange(this.EditorWindow);
		range.MoveToSelection();
...
FCK.CreateLink = function(A, B) {
	var C = [];
	if (FCKSelection.GetSelection().isCollapsed) return C;
...
FCKTools.DisableSelection = function(A) {
	if (FCKBrowserInfo.IsGecko) A.style.MozUserSelect = 'none';
	else if (FCKBrowserInfo.IsSafari) A.style.KhtmlUserSelect = 'none';
	else A.style.userSelect = 'none';
};
...
FCKTools.GetElementPosition = function(A, B) {
	...
	while (A) {
		...
		if (F && F != 'static' && A.style.zIndex != FCKConfig.FloatingPanelsZIndex) break;
		...
		if (A.offsetParent) A = A.offsetParent;
			...
			if (D != C) {
				A = D.frameElement;
				E = null;
				if (A) D = FCKTools.GetElementWindow(A);
			} else {
				c.X += A.scrollLeft;
				c.Y += A.scrollTop;
				break;
...
if (FCKBrowserInfo.IsGecko) {
	FCKXHtml.TagProcessors['link'] = function(A, B) {
		if (B.href.substr(0, 9).toLowerCase() == 'chrome://') return false;
		return A;
	}
};
...
FCKDomRange.prototype.MoveToSelection = function() {
	...
	if (A && A.rangeCount > 0) {
		...
		this._UpdateElementInfo();
	} else if (this.Window.document) this.MoveToElementStart(this.Window.document.body);
};
FCKDomRange.prototype.Select = function() {
	var A = this._Range;
	if (A) {
		var B = A.startContainer;
		if (A.collapsed && B.nodeType == 1 && B.childNodes.length == 0) B.appendChild(A._Document.createTextNode(''));
	...
	if (this.GetType() == 'Control') return FCKSelection.GetSelectedElement().parentNode;
		...
		if (A) {
			if (A.anchorNode && A.anchorNode == A.focusNode) {
				var B = A.getRangeAt(0);
				if (B.collapsed || B.startContainer.nodeType == 3) return A.anchorNode.parentNode;
				else return A.anchorNode;
			};
...
FCKSelection.GetBoundaryParentElement = function(A) {
	if (!FCK.EditorWindow) return null;
	if (this.GetType() == 'Control') return FCKSelection.GetSelectedElement().parentNode;
	else {
...
FCKSelection.HasAncestorNode = function(A) {
	...
	while (B) {
		if (B.nodeType == 1 && B.nodeName.IEquals(A)) return true;
		B = B.parentNode;
	};
	return false;
};
FCKSelection.MoveToAncestorNode = function(A) {
	...
	while (C) {
		if (C.nodeName.IEquals(A)) return C;
		C = C.parentNode;
	};
	return null;
};
...
FCKTableHandler.GetSelectedCells = function() {
	...
	for (var i = 0; i < B.rangeCount; i++) {
		...
		var E;
		if (D.startContainer.tagName.Equals('TD', 'TH')) E = D.startContainer;
		else E = D.startContainer.childNodes[D.startOffset];
		if (E.nodeName.Equals('TD', 'TH')) A[A.length] = E;
	};
	return A;
};
var FCKXml = function() {
...
FCKXml.prototype = {
	LoadUrl: function(A) {
		...
		if (C.status == 200 || C.status == 304 || (C.status == 0 && C.readyState == 4)) {
			B = C.responseXML;
			if (!B) B = (new DOMParser()).parseFromString(C.responseText, 'text/xml');
		} else B = null;
		if (B) {
			try {
				var D = B.firstChild;
...
(function() {
	...
	function GetStatusLabel() {
		if (!D) return '<b>Enable SCAYT</b>';
		return C ? '<b>Disable SCAYT</b>' : '<b>Enable SCAYT</b>';
	};
