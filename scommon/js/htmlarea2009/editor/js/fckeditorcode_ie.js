...
function FCKIECleanup_Cleanup() {
	if (!this._FCKCleanupObj || (FCKConfig.MsWebBrowserControlCompat && !window.FCKUnloadFlag)) return;
...
(function(A) {
	...
	if (A.IsSafari) A.IsSafari3 = (parseFloat(s.match(/ applewebkit\/(\d+)/)[1]) < 526);
})(FCKBrowserInfo);
var FCKURLParams = {};
(function() {
...
FCKEvents.prototype.AttachEvent = function(A, B) {
	var C;
	if (!(C = this._RegisteredEvents[A])) this._RegisteredEvents[A] = [B];
	else {
...
FCKDataProcessor.prototype = {
	ConvertToHtml: function(A) {
		if (FCKConfig.FullPage) {
			...
			if (!FCKRegexLib.HasBodyTag.test(A)) A = '<body>' + A + '</body>';
			if (!FCKRegexLib.HtmlOpener.test(A)) A = '<html dir="' + FCKConfig.ContentLangDirection + '">' + A + '</html>';
			if (!FCKRegexLib.HeadOpener.test(A)) A = A.replace(FCKRegexLib.HtmlOpener, '$&<head><title></title></head>');
			return A;
		} else {
			var B = FCKConfig.DocType + '<html dir="' + FCKConfig.ContentLangDirection + '"';
			if (FCKBrowserInfo.IsIE && FCKConfig.DocType.length > 0 && !FCKRegexLib.Html4DocType.test(FCKConfig.DocType)) B += ' style="overflow-y: scroll"';
	...
	ConvertToDataFormat: function(A, B, C, D) {
		...
		if (C && FCKRegexLib.EmptyOutParagraph.test(E)) return '';
		return E;
	},
	FixHtml: function(A) {
		return A;
...
var FCK = {
	...
	StartupValue: '',
	IsDirty: function() {
		if (this.EditMode == 1) return (this.StartupValue != this.EditingArea.Textarea.value);
		else {
			if (!this.EditorDocument) return false;
	...
	StartEditor: function() {
		...
		if (FCKBrowserInfo.IsGecko) this.AttachToOnSelectionChange(this._ExecCheckEmptyBlock);
	},
	Focus: function() {
		...
		while (D) {
			...
			switch (D.nodeType) {
				case 1:
					...
					if (!FCKListsLib.BlockElements[G] && G != 'li' && !D.getAttribute('_fckfakelement') && D.getAttribute('_moz_dirty') == null) F = true;
					break;
				case 3:
			...
			if (F) {
				var H = D.parentNode;
				if (!E) E = H.insertBefore(B.createElement(A), D);
	...
	GetData: function(A) {
		...
		if (FCKBrowserInfo.IsIE) D = D.replace(FCKRegexLib.ToReplace, '$1');
		if (C) {
			if (FCK.DocTypeDeclaration && FCK.DocTypeDeclaration.length > 0) D = FCK.DocTypeDeclaration + '\n' + D;
			if (FCK.XmlDeclaration && FCK.XmlDeclaration.length > 0) D = FCK.XmlDeclaration + '\n' + D;
		};
		D = FCKConfig.ProtectedSource.Revert(D);
		setTimeout(function() {
			FCK.Events.FireEvent("OnAfterGetData");
		}, 0);
		return D;
	},
	UpdateLinkedField: function() {
		var A = FCK.GetXHTML(FCKConfig.FormatOutput);
		if (FCKConfig.HtmlEncodeOutput) A = FCKTools.HTMLEncode(A);
	...
	RegisterDoubleClickHandler: function(A, B) {
		...
		C = C.toUpperCase();
		var D;
		if (!(D = FCK.RegisteredDoubleClickHandlers[C])) FCK.RegisteredDoubleClickHandlers[C] = [A];
		else {
	...
	ProtectTags: function(A) {
		...
		if (FCKBrowserInfo.IsIE) B += B.length > 0 ? '|ABBR|XML|EMBED|OBJECT' : 'ABBR|XML|EMBED|OBJECT';
		var C;
		if (B.length > 0) {
			C = new RegExp('<(' + B + ')(?!\w|:)', 'gi');
			A = A.replace(C, '<FCK:$1');
	...
	SetData: function(A, B) {
		...
		if (FCK.EditMode == 0) {
			...
			var C = '';
			if (!FCKConfig.FullPage) C += _FCK_GetEditorAreaStyleTags();
			if (FCKBrowserInfo.IsIE) C += FCK._GetBehaviorsStyle();
			else if (FCKConfig.ShowBorders) C += FCKTools.GetStyleHtml(FCK_ShowTableBordersCSS, true);
	...
	GetNamedCommandState: function(A) {
		try {
			if (FCKBrowserInfo.IsSafari && FCK.EditorWindow && A.IEquals('Paste')) return 0;
			if (!FCK.EditorDocument.queryCommandEnabled(A)) return -1;
			else {
	...
	Paste: function(A) {
		if (FCK.Status != 2 || !FCK.Events.FireEvent('OnPaste')) return false;
	...
	Preview: function() {
		var A;
		if (FCKConfig.FullPage) {
			if (FCK.TempBaseTag.length > 0) A = FCK.TempBaseTag + FCK.GetXHTML();
			else A = FCK.GetXHTML();
		} else {
	...
	InsertElement: function(A) {
		if (typeof A == 'string') A = this.EditorDocument.createElement(A);
		...
		if (FCKListsLib.BlockElements[B] != null) {
			if (C.StartBlock) {
				if (C.CheckStartOfBlock()) C.MoveToPosition(C.StartBlock, 3);
				else if (C.CheckEndOfBlock()) C.MoveToPosition(C.StartBlock, 4);
				else C.SplitBlock();
			};
			C.InsertNode(A);
			...
			if (FCKListsLib.EmptyElements[B] == null) C.MoveToElementEditStart(A);
			else if (D) C.MoveToElementEditStart(D);
			else C.MoveToPosition(A, 4);
	...
	_IsFunctionKey: function(A) {
		if (A >= 16 && A <= 20) return true;
		if (A == 27 || (A >= 33 && A <= 40)) return true;
		if (A == 45) return true;
		return false;
	},
	_KeyDownListener: function(A) {
		if (!A) A = FCK.EditorWindow.event;
		if (FCK.EditorWindow) {
			if (!FCK._IsFunctionKey(A.keyCode) && !(A.ctrlKey || A.metaKey) && !(A.keyCode == 46)) FCK._KeyDownUndo();
		};
		return true;
	},
	...
	_TabKeyHandler: function(A) {
		...
		if (B == 9 && FCK.EditMode != 0) {
			if (FCKBrowserInfo.IsIE) {
				...
				if (C.parentElement() != FCK.EditingArea.Textarea) return true;
				C.text = '\t';
				C.select();
			} else {
				var a = [];
...
function _FCK_MouseEventsListener(A) {
	if (!A) A = window.event;
	if (A.type == 'mousedown') FCK.MouseDownFlag = true;
	else if (A.type == 'mouseup') FCK.MouseDownFlag = false;
	else if (A.type == 'mousemove') FCK.Events.FireEvent('OnMouseMove', A);
};

function _FCK_PaddingNodeListener() {
	if (FCKConfig.EnterMode.IEquals('br')) return;
...
function _FCK_EditingArea_OnLoad() {
	...
	if (FCKBrowserInfo.IsIE) FCKTempBin.ToElements();
	...
	if (FCKBrowserInfo.IsSafari) {
		FCKTools.AddEventListener(FCK.EditorDocument, 'paste', function(evt) {
			...
			var B = FCK.EditorDocument.createTextNode('\ufeff');
			var C = FCK.EditorDocument.createElement('a');
			C.id = 'fck_paste_padding';
			C.innerHTML = '&#65279;';
			A.MoveToSelection();
			A.DeleteContents();
			A.InsertNode(B);
			A.Collapse();
			A.InsertNode(C);
			A.MoveToPosition(C, 3);
			A.Select();
			setTimeout(function() {
				B.parentNode.removeChild(B);
				C = FCK.EditorDocument.getElementById('fck_paste_padding');
	...
	if (FCKBrowserInfo.IsSafari) {
		...
		FCKTools.AddEventListener(FCK.EditorDocument, 'keyup', D);
	};
	debugger;
...
var FCKFocusManager = FCK.FocusManager = {
	...
	AddWindow: function(A, B) {
		var C;
		if (FCKBrowserInfo.IsIE) C = A.nodeType == 1 ? A : A.frameElement ? A.frameElement : A.document;
	...
	RemoveWindow: function(A) {
		if (FCKBrowserInfo.IsIE) oTarget = A.nodeType == 1 ? A : A.frameElement ? A.frameElement : A.document;
		else oTarget = A.document;
...
function FCKFocusManager_FireOnBlur() {
	if (FCKFocusManager.IsLocked) FCKFocusManager._HasPendingBlur = true;
	else {
		FCK.HasFocus = false;
...
function FCKFocusManager_Win_OnFocus_Area() {
	if (FCKFocusManager._IsFocusing) return;
...
FCK._GetBehaviorsStyle = function() {
	if (!FCK._BehaviorsStyle) {
		...
		if (FCKConfig.ShowBorders) B = 'url(' + A + 'css/behaviors/showtableborders.htc)';
		...
		C += ' { behavior: url(' + A + 'css/behaviors/disablehandles.htc) ; }';
		if (B.length > 0) C += 'TABLE { behavior: ' + B + ' ; }';
		C += '</style>';
...
function Doc_OnSelectionChange() {
	if (!FCK.IsSelectionChangeLocked && FCK.EditorDocument) FCK.Events.FireEvent("OnSelectionChange");
};
...
function Doc_OnDrop() {
	...
	if (FCKConfig.ForcePasteAsPlainText) {
		...
		if (FCK._CheckIsPastingEnabled() || FCKConfig.ShowDropDialog) FCK.PasteAsPlainText(A.dataTransfer.getData('Text'));
		A.returnValue = false;
		A.cancelBubble = true;
	}
};
FCK.InitializeBehaviors = function(A) {
	...
	this.EditorDocument.attachEvent("onbeforedeactivate", function() {
		FCKSelection.Save();
	});
...
FCK.InsertHtml = function(A) {
	...
	var C = FCK.EditorDocument.getElementById('__fakeFCKRemove__');
	if (C.parentNode.childNodes.length == 1) C = C.parentNode;
	C.removeNode(true);
	FCKDocumentProcessor.Process(FCK.EditorDocument);
...
FCK._ExecPaste = function() {
	...
	if (A === false) FCKTools.RunFunction(FCKDialog.OpenDialog, FCKDialog, ['FCKDialog_Paste', FCKLang.Paste, 'dialog/fck_paste.html', 400, 330, 'Security']);
	else {
...
FCK.PasteAsPlainText = function(A) {
	...
	var B = null;
	if (!A) B = clipboardData.getData("Text");
	else B = A;
	if (B && B.length > 0) {
		B = FCKTools.HTMLEncode(B);
...
FCK.CreateLink = function(A, B) {
	var C = [];
	var D = FCKSelection.GetType() == 'Control';
	var E = D && FCKSelection.GetSelectedElement();
	if (!(D && !FCKTools.GetElementAscensor(E, 'a'))) FCK.ExecuteNamedCommand('Unlink', null, false, !!B);
	if (A.length > 0) {
		if (D) {
			var F = this.EditorDocument.createElement('A');
			F.href = A;
			var G = E;
		...
		var H = 'javascript:void(0);/*' + (new Date().getTime()) + '*/';
		FCK.ExecuteNamedCommand('CreateLink', H, false, !!B);
		var I = this.EditorDocument.links;
		for (i = 0; i < I.length; i++) {
			var F = I[i];
			if (F.getAttribute('href', 2) == H) {
				var K = F.innerHTML;
				F.href = A;
				F.innerHTML = K;
				var L = F.lastChild;
				if (L && L.nodeName == 'BR') {
					FCKDomTools.InsertAfterNode(F, F.removeChild(L));
				};
				C.push(F);
			}
		}
...
if (document.location.protocol == 'file:') {
	...
	if (FCKBrowserInfo.IsOpera) sFullProtocol += 'localhost/';
...
FCKConfig.ProcessHiddenField = function() {
	...
	for (var i = 0; i < B.length; i++) {
		...
		else if (E.length > 0 && !isNaN(E)) this.PageConfig[D] = parseFloat(E);
...
function FCKConfig_PreProcess() {
	...
	if (!FCKConfig.SkinEditorCSS || FCKConfig.SkinEditorCSS.length == 0) FCKConfig.SkinEditorCSS = FCKConfig.SkinPath + 'fck_editor.css';
	if (!FCKConfig.SkinDialogCSS || FCKConfig.SkinDialogCSS.length == 0) FCKConfig.SkinDialogCSS = FCKConfig.SkinPath + 'fck_dialog.css';
};
...
FCKConfig.GetBodyAttributes = function() {
	var A = '';
	if (this.BodyId && this.BodyId.length > 0) A += ' id="' + this.BodyId + '"';
	if (this.BodyClass && this.BodyClass.length > 0) A += ' class="' + this.BodyClass + '"';
	return A;
};
FCKConfig.ApplyBodyAttributes = function(A) {
	if (this.BodyId && this.BodyId.length > 0) A.id = FCKConfig.BodyId;
	if (this.BodyClass && this.BodyClass.length > 0) A.className += ' ' + FCKConfig.BodyClass;
};
var FCKDebug = {
	Output: function() {},
...
var FCKDomTools = {
	...
	MoveNode: function(A, B, C) {
		if (C) B.insertBefore(FCKDomTools.RemoveNode(A), B.firstChild);
		else B.appendChild(FCKDomTools.RemoveNode(A));
	},
	TrimNode: function(A) {
		this.LTrimNode(A);
		this.RTrimNode(A);
	...
	GetPreviousSourceElement: function(A, B, C, D) {
		...
		else return this.GetPreviousSourceElement(A.parentNode, B, C, D);
		while (A) {
			if (A.nodeType == 1) {
			...
			else return this.GetPreviousSourceElement(A, B, C, D);
		};
		return null;
	},
	GetNextSourceElement: function(A, B, C, D, E) {
		while ((A = this.GetNextSourceNode(A, E))) {
			if (A.nodeType == 1) {
				if (C && A.nodeName.IEquals(C)) break;
				if (D && A.nodeName.IEquals(D)) return this.GetNextSourceElement(A, B, C, D);
				return A;
			} else if (B && A.nodeType == 3 && A.nodeValue.RTrim().length > 0) break;
		};
		return null;
	...
	GetNextSourceNode: function(A, B, C, D) {
		...
		if (!B && A.firstChild) E = A.firstChild;
			...
			E = A.nextSibling;
			if (!E && (!D || D != A.parentNode)) return this.GetNextSourceNode(A.parentNode, true, C, D);
		};
		if (C && E && E.nodeType != C) return this.GetNextSourceNode(E, false, C, D);
		return E;
	},
	GetPreviousSourceNode: function(A, B, C, D) {
		...
		if (!B && A.lastChild) E = A.lastChild;
			...
			if (D && A == D) return null;
			E = A.previousSibling;
			if (!E && (!D || D != A.parentNode)) return this.GetPreviousSourceNode(A.parentNode, true, C, D);
		};
		if (C && E && E.nodeType != C) return this.GetPreviousSourceNode(E, false, C, D);
		return E;
	},
	...
	HasAttributes: function(A) {
		...
		for (var i = 0; i < B.length; i++) {
			...
			if (B[i].specified) return true;
		};
		return false;
	},
	RemoveAttribute: function(A, B) {
		if (FCKBrowserInfo.IsIE && B.toLowerCase() == 'class') B = 'className';
		return A.removeAttribute(B, 0);
	},
	RemoveAttributes: function(A, B) {
		for (var i = 0; i < B.length; i++) this.RemoveAttribute(A, B[i]);
	},
	...
	ClearElementJSProperty: function(A, B) {
		if (FCKBrowserInfo.IsIE) A.removeAttribute(B);
		else delete A[B];
	},
	...
	ClearElementMarkers: function(A, B, C) {
		...
		for (var j in A[D]['markers']) this.ClearElementJSProperty(B, j);
		if (C) delete A[D];
	},
	ClearAllMarkers: function(A) {
		for (var i in A) this.ClearElementMarkers(A, A[i]['element'], true);
	},
	ListToArray: function(A, B, C, D, E) {
		if (!A.nodeName.IEquals(['ul', 'ol'])) return [];
		if (!D) D = 0;
		if (!C) C = [];
		for (var i = 0; i < A.childNodes.length; i++) {
			var F = A.childNodes[i];
			if (!F.nodeName.IEquals('li')) continue;
			var G = {
				'parent': A,
				'indent': D,
				'contents': []
			...
			if (!E) {
				...
				if (G.grandparent && G.grandparent.nodeName.IEquals('li')) G.grandparent = G.grandparent.parentNode;
			} else G.grandparent = E;
			if (B) this.SetElementMarker(B, F, '_FCK_ListArray_Index', C.length);
			C.push(G);
			for (var j = 0; j < F.childNodes.length; j++) {
				var H = F.childNodes[j];
				if (H.nodeName.IEquals(['ul', 'ol'])) this.ListToArray(H, B, C, D + 1, G.grandparent);
	...
	ArrayToList: function(A, B, C) {
		...
		while (true) {
			...
			if (J.indent == H) {
				...
				var I;
				if (J.grandparent.nodeName.IEquals(['ul', 'ol'])) I = D.createElement('li');
				else {
					if (FCKConfig.EnterMode.IEquals(['div', 'p']) && !J.grandparent.nodeName.IEquals('td')) I = D.createElement(FCKConfig.EnterMode);
				...
				if (FCKBrowserInfo.IsGeckoLike && I.nodeName.IEquals(['div', 'p'])) FCKTools.AppendBogusBr(I);
				E.appendChild(I);
				F = null;
				G++;
			} else return null;
		...
		if (B) {
			var N = E.firstChild;
			while (N) {
				if (N.nodeType == 1) this.ClearElementMarkers(B, N);
	...
	CheckIsEmptyElement: function(A, B) {
		...
		while (C) {
			if (C.nodeType == 1) {
				if (D || !FCKListsLib.InlineNonEmptyElements[C.nodeName.toLowerCase()]) return false;
				if (!B || B(C) === true) D = C;
	...
	GetCurrentElementStyle: function(A, B) {
		if (FCKBrowserInfo.IsIE) return A.currentStyle[B];
	...
	GetPositionedAncestor: function(A) {
		...
		while (B != FCKTools.GetElementDocument(B).documentElement) {
			if (this.GetCurrentElementStyle(B, 'position') != 'static') return B;
			if (B == FCKTools.GetElementDocument(B).documentElement && currentWindow != w) B = currentWindow.frameElement;
			else B = B.parentNode;
		};
		return null;
	},
	ScrollIntoView: function(A, B) {
		...
		if (E > 0 && (E > G || E < G - D)) C.scrollTo(0, E);
	},
	...
	GetSelectedDivContainers: function() {
		...
		var E = C;
		if (C == D) {
			while (D.nodeType == 1 && D.lastChild) D = D.lastChild;
		...
		while (E && E != D) {
			if (E.nodeType != 3 || !/^[ \t\n]*$/.test(E.nodeValue)) {
				...
				var G = F.BlockLimit;
				if (G && G.nodeName.IEquals('div') && A.IndexOf(G) == -1) A.push(G);
			};
...
FCKTools.FixCssUrls = function(A, B) {
	...
	return B.replace(/url\s*\(([\s'"]*)(.*?)([\s"']*)\)/g, function(match, opener, path, closer) {
		...
		else return 'url(' + opener + A + path + closer + ')';
	});
};
FCKTools._GetUrlFixedCss = function(A, B) {
	var C = A.match(/^([^|]+)\|([\s\S]*)/);
	if (C) return FCKTools.FixCssUrls(C[1], C[2]);
	else return A;
};
FCKTools.AppendStyleSheet = function(A, B) {
	...
	if (typeof(B) == 'string') {
		...
		} else return [this.AppendStyleString(A, FCKTools._GetUrlFixedCss(B))];
	} else {
		var C = [];
...
FCKTools.GetStyleHtml = (function() {
	...
	return function(cssFileOrArrayOrDef, markTemp) {
		...
		if (typeof(cssFileOrArrayOrDef) == 'string') {
			...
			} else return A(this._GetUrlFixedCss(cssFileOrArrayOrDef), markTemp);
		} else {
			var E = '';
			for (var i = 0; i < cssFileOrArrayOrDef.length; i++) E += C(cssFileOrArrayOrDef[i], markTemp);
			return E;
		}
	}
})();
...
FCKTools.ProcessLineBreaks = function(A, B, C) {
	...
	if (D == 'p') this._ProcessLineBreaksForPMode(A, C, F, H, E);
	else if (D == 'div') this._ProcessLineBreaksForDivMode(A, C, F, H, E);
	else if (D == 'br') this._ProcessLineBreaksForBrMode(A, C, F, H, E);
	return E.join("");
};
...
FCKTools.AppendBogusBr = function(A) {
	...
	if (!B || (B.getAttribute('type', 2) != '_moz' && B.getAttribute('_moz_dirty') == null)) {
		...
		if (FCKBrowserInfo.IsOpera) A.appendChild(C.createTextNode(''));
		else A.appendChild(this.CreateBogusBR(C));
	}
};
...
FCKTools.GetVoidUrl = function() {
	...
	if (FCKBrowserInfo.IsIE) {
		if (FCKBrowserInfo.IsIE7 || !FCKBrowserInfo.IsIE6) return "";
...
FCKTools.CreateXmlObject = function(A) {
	...
	switch (A) {
		case 'XmlHttp':
			if (document.location.protocol != 'file:') try {
...
function FCKeditorAPI_Cleanup() {
	if (window.FCKConfig && FCKConfig.MsWebBrowserControlCompat && !window.FCKUnloadFlag) return;
	delete FCKeditorAPI.Instances[FCK.Name];
};

function FCKeditorAPI_ConfirmCleanup() {
	if (window.FCKConfig && FCKConfig.MsWebBrowserControlCompat) window.FCKUnloadFlag = true;
};
...
function _FCKImagePreloader_OnImage(A, B) {
	if ((--B._PreloadCount) == 0 && B.OnComplete) B.OnComplete();
};
var FCKRegexLib = {
	AposEntity: /&apos;/gi,
...
var FCKLanguageManager = FCK.Language = {
	...
	Initialize: function() {
		if (this.AvailableLanguages[FCKConfig.DefaultLanguage]) this.DefaultLanguage = FCKConfig.DefaultLanguage;
...
FCKXHtml.GetXHTML = function(A, B, C) {
	...
	if (FCKBrowserInfo.IsIE) FCKXHtml._RemoveXHtmlJobProperties(A);
	var E = this._GetMainXmlString();
	this.XML = null;
	if (FCKBrowserInfo.IsSafari) E = E.replace(/^<xhtml.*?>/, '<xhtml>');
	E = E.substr(7, E.length - 15).Trim();
	if (FCKConfig.DocType.length > 0 && FCKRegexLib.HtmlDocType.test(FCKConfig.DocType)) E = E.replace(FCKRegexLib.SpaceNoClose, '>');
	else E = E.replace(FCKRegexLib.SpaceNoClose, ' />');
...
FCKXHtml._AppendChildNodes = function(A, B, C) {
	...
	if (C && B.tagName && B.tagName.toLowerCase() != 'pre') {
		...
		if (FCKConfig.FillEmptyBlocks) {
			var E = A.lastChild;
			if (E && E.nodeType == 1 && E.nodeName == 'br') this._AppendEntity(A, this._NbspEntity);
		}
	};
	if (A.childNodes.length == 0) {
		...
		var F = A.nodeName;
		if (FCKListsLib.InlineChildReqElements[F]) return null;
		if (!FCKListsLib.EmptyElements[F]) A.appendChild(this.XML.createTextNode(''));
	};
	return A;
};
...
FCKXHtml._AppendAttributes = function(A, B, C, D) {
	...
	for (var n = 0; n < E.length; n++) {
		...
		if (F.specified) {
			...
			} else if (G.indexOf('on') == 0) H = F.nodeValue;
	...
	if (bHasStyle || B.style.cssText.length > 0) {
...
FCKXHtml._RemoveXHtmlJobProperties = function(A) {
	if (!A || !A.nodeType || A.nodeType != 1) return;
	if (typeof A._fckxhtmljob == 'undefined' && A.tagName !== 'BODY') return;
	A.removeAttribute('_fckxhtmljob');
	if (A.hasChildNodes()) {
		var B = A.childNodes;
		for (var i = B.length - 1; i >= 0; i--) {
			var C = B[i];
			if (C.parentNode == A) FCKXHtml._RemoveXHtmlJobProperties(C);
		}
	}
};
...
FCKXHtml.TagProcessors['map'] = function(A, B) {
	if (!A.attributes.getNamedItem('name')) {
		var C = B.name;
		if (C) FCKXHtml._AppendAttribute(A, 'name', C);
	};
	A = FCKXHtml._AppendChildNodes(A, B, true);
	return A;
};
...
FCKUndo._SelectBookmark = function(A) {
	...
	if (A instanceof Object) {
		if (FCKBrowserInfo.IsIE) B.MoveToBookmark(A[0]);
		else B.MoveToBookmark2(A);
		try {
			B.Select();
		} catch (e) {
...
FCKUndo.SaveUndoStep = function() {
	if (FCK.EditMode != 0 || this.SaveLocked) return;
	if (this.SavedData.length) this.Changed = true;
	var A = FCK.EditorDocument.body.innerHTML;
	var B = this._GetBookmark();
	this.SavedData = this.SavedData.slice(0, this.CurrentIndex + 1);
	if (this.CurrentIndex > 0 && A == this.SavedData[this.CurrentIndex][0] && this._CheckIsBookmarksEqual(B, this.SavedData[this.CurrentIndex][1])) return;
	else if (this.CurrentIndex == 0 && this.SavedData.length && A == this.SavedData[0][0]) {
		this.SavedData[0][1] = B;
		return;
	};
	if (this.CurrentIndex + 1 >= FCKConfig.MaxUndoLevels) this.SavedData.shift();
...
FCKUndo._ApplyUndoLevel = function(A) {
	...
	if (!B) return;
	if (FCKBrowserInfo.IsIE) {
		if (B[1] && B[1][1]) FCK.SetInnerHtml(B[1][1]);
		else FCK.SetInnerHtml(B[0]);
	} else FCK.EditorDocument.body.innerHTML = B[0];
	...
	if (this.Mode == 0) {
		...
		if (FCKBrowserInfo.IsIE) A = A.replace(/(<base[^>]*?)\s*\/?>(?!\s*<\/base>)/gi, '$1></base>');
			...
			if (E && F) {
				...
				A = E[1] + '&nbsp;' + F[1];
				if (FCKBrowserInfo.IsGecko && (G.length == 0 || FCKRegexLib.EmptyParagraph.test(G))) G = '<br type="_moz">';
				this._BodyHTML = G;
...
function FCKEditingArea_Document_AttributeNodeModified(A) {
	...
	if (B._timer) window.clearTimeout(B._timer);
...
FCKEditingArea.prototype.Focus = function() {
	try {
		if (this.Mode == 0) {
			if (FCKBrowserInfo.IsIE) this._FocusIE();
...
FCKStyle.prototype = {
	GetType: function() {
		...
		var B = this.Element;
		if (B == '#' || FCKListsLib.StyleBlockElements[B]) A = 0;
		else if (FCKListsLib.StyleObjectElements[B]) A = 2;
		else A = 1;
	...
	RemoveFromRange: function(A, B, C) {
			...
			for (var i = 0; i < I.Elements.length; i++) {
				...
				if (this.CheckElementRemovable(O)) {
					if (L && !FCKDomTools.CheckIsEmptyElement(O, function(el) {
								...
								if (FCKDomTools.HasAttribute(O, Q)) {
									switch (Q) {
										case 'style':
											...
											break;
										case 'class':
											if (FCKDomTools.GetAttributeValue(O, Q) != this.GetFinalAttributeValue(Q)) continue;
										default:
						...
						if (this.GetType() == 1) this._RemoveNoAttribElement(O);
					}
				} else if (L) J.push(O);
				...
				if (M && (!L || (i == I.Elements.length - 1))) {
					...
					if (K) FCKDomTools.InsertAfterNode(M, R);
					else M.parentNode.insertBefore(R, M);
					L = false;
					M = null;
				}
			};
			if (B) A.SelectBookmark(D);
			if (C) A.MoveToBookmark(D);
			return;
		};
		...
		for (var i = 1; i < X.length; i++) {
			O = X[i];
			if (O == I.Block || O == I.BlockLimit) break;
			if (this.CheckElementRemovable(O)) FCKDomTools.BreakParent(T, O, A);
		};
		...
		for (var i = 1; i < X.length; i++) {
			...
			if (this.CheckElementRemovable(O)) FCKDomTools.BreakParent(U, O, A);
		};
						...
						if (FCKDomTools.HasAttribute(Z, Q)) {
							switch (Q) {
								case 'style':
									...
									break;
								case 'class':
									if (FCKDomTools.GetAttributeValue(Z, Q) != this.GetFinalAttributeValue(Q)) continue;
								default:
	...
	CheckElementRemovable: function(A, B) {
		...
		if (C == this.Element) {
			if (!B && !FCKDomTools.HasAttributes(A)) return true;
			var D = this._GetAttribsForComparison();
			var E = (D._length == 0);
			for (var F in D) {
				if (F == '_length') continue;
		...
		if (G) {
			...
			for (var i = 0; i < D.length; i++) {
				...
				if (FCKDomTools.HasAttribute(A, H)) {
					var I = D[i][1];
					if (I == null || (typeof I == 'string' && FCKDomTools.GetAttributeValue(A, H) == I) || I.test(FCKDomTools.GetAttributeValue(A, H))) return true;
				}
			}
		};
		return false;
	...
	CheckActive: function(A) {
		switch (this.GetType()) {
			...
			case 1:
				...
				for (var i = 0; i < B.length; i++) {
					var C = B[i];
					if (C == A.Block || C == A.BlockLimit) continue;
					if (this.CheckElementRemovable(C, true)) return true;
				}
		};
		return false;
	},
				...
				if (FCKDomTools.HasAttribute(E, F)) {
					switch (F) {
						case 'style':
							...
							break;
						case 'class':
							if (FCKDomTools.GetAttributeValue(E, F) != this.GetFinalAttributeValue(F)) continue;
						default:
	...
	_RemoveStylesFromElement: function(A) {
		...
		B = B.replace(D, '').Trim();
		if (B.length == 0 || B == ';') FCKDomTools.RemoveAttribute(A, 'style');
		else A.style.cssText = B.replace(D, '');
	},
		...
		if (C) {
			for (var i = 0; i < C.length; i++) {
				...
				if (FCKDomTools.HasAttribute(A, D)) {
					var E = C[i][1];
					if (E == null || (E.test && E.test(FCKDomTools.GetAttributeValue(A, D))) || (typeof E == 'string' && FCKDomTools.GetAttributeValue(A, D) == E)) FCKDomTools.RemoveAttribute(A, D);
				}
			}
		}
	},
	...
	BuildElement: function(A, B) {
		...
		if (D) {
			for (var F in D) {
				...
				if (F.toLowerCase() == 'class') C.className = E;
				else C.setAttribute(F, E);
			}
		};
		if (this._GetStyleText().length > 0) C.style.cssText = this.GetFinalStyleValue();
		return C;
	},
	...
	GetFinalAttributeValue: function(A) {
		...
		var B = B ? B[A] : null;
		if (!B && A == 'style') return this.GetFinalStyleValue();
		if (B && this._Variables) B = B.Replace(FCKRegexLib.StyleVariableAttName, this._GetVariableReplace, this);
		return B;
	},
	...
	_FromPre: function(A, B, C) {
		...
		D = D.replace(/^[ \t]+|[ \t]+$/g, function(match, offset, s) {
			...
			else if (offset == 0) return new Array(match.length).join('&nbsp;') + ' ';
	...
	_ToPre: function(A, B, C) {
		...
		var F = [];
		E.Each(function(isTag, value) {
			if (!isTag) value = value.replace(/([ \t\n\r]+|&nbsp;)/g, ' ');
			else if (isTag && value == '<br />') value = '\n';
			F.push(value);
		});
		if (FCKBrowserInfo.IsIE) {
		...
		} else C.innerHTML = F.join('');
		return C;
	},
	_CheckAndMergePre: function(A, B) {
		if (A != FCKDomTools.GetPreviousSourceElement(B, true)) return;
		var C = A.innerHTML.replace(/\n$/, '') + '\n\n' + B.innerHTML.replace(/^\n/, '');
		if (FCKBrowserInfo.IsIE) B.outerHTML = '<pre>' + C + '</pre>';
		else B.innerHTML = C;
	...
	_ApplyBlockStyle: function(A, B, C) {
		...
		while ((F = E.GetNextParagraph())) {
			...
			else if (M) I = this._FromPre(G, F, I);
			else FCKDomTools.MoveChildren(F, I);
			F.parentNode.insertBefore(I, F);
			FCKDomTools.RemoveNode(F);
			if (J) {
				if (H) this._CheckAndMergePre(H, I);
				H = I;
			} else if (M) this._CheckAndSplitPre(I);
		};
		if (B) A.SelectBookmark(D);
		if (C) A.MoveToBookmark(D);
	},
		...
		while (M) {
			...
			if (!P || G[P]) {
				if ((FCK.DTD[M.parentNode.nodeName.toLowerCase()] || FCK.DTD.span)[F] || !FCK.DTD[F]) {
					...
					if (O != 1 || M.childNodes.length == 0) {
						...
						A.SetEnd(Q, 4);
						if (Q == Q.parentNode.lastChild && !G[Q.parentNode.nodeName.toLowerCase()]) N = true;
					} else {
						A.SetEnd(M, 3);
					}
				} else N = true;
			...
			if (N && A.CheckHasRange() && !A.CheckIsCollapsed()) {
				...
				if (I.innerHTML.RTrim().length > 0) {
					...
					if (!FCKBrowserInfo.IsIE) I.normalize();
				};
				A.Release(true);
			}
		};
		this._FixBookmarkStart(K);
		if (B) A.SelectBookmark(J);
		if (C) A.MoveToBookmark(J);
	},
	_FixBookmarkStart: function(A) {
		...
		while ((B = A.nextSibling)) {
			if (B.nodeType == 1 && FCKListsLib.InlineNonEmptyElements[B.nodeName.toLowerCase()]) {
				if (!B.firstChild) FCKDomTools.RemoveNode(B);
				else FCKDomTools.MoveNode(A, B, true);
				continue;
			};
	...
	_MergeSiblings: function(A, B) {
		if (!A || A.nodeType != 1 || !FCKListsLib.InlineNonEmptyElements[A.nodeName.toLowerCase()]) return;
	...
	_MergePreviousSibling: function(A, B) {
		...
		if (C && C.nodeType == 1 && C.nodeName == A.nodeName) {
			...
			if (this._CheckAttributesMatch(C, B)) {
				var E = A.firstChild;
				if (D) FCKDomTools.MoveNode(A.previousSibling, A, true);
				FCKDomTools.MoveChildren(C, A, true);
				FCKDomTools.RemoveNode(C);
				if (E) this._MergePreviousSibling(E);
			}
		}
	},
	...
	_GetOverridesForComparison: function() {
		...
		if (B) {
			if (!FCKTools.IsArray(B)) B = [B];
			for (var i = 0; i < B.length; i++) {
				var C = B[i];
				var D;
				var E;
				var F;
				if (typeof C == 'string') D = C.toLowerCase();
				else {
	...
	_CheckAttributesMatch: function(A, B) {
		...
		for (var i = 0; i < C.length; i++) {
			...
			if (E.specified) {
				...
				var G = B[F];
				if (!G) break;
				if (G != FCKDomTools.GetAttributeValue(A, E).toLowerCase()) break;
				D++;
			}
		};
		return (D == B._length);
...
var FCKStyles = FCK.Styles = {
	...
	ApplyStyle: function(A) {
		if (typeof A == 'string') A = this.GetStyles()[A];
		if (A) {
			if (A.GetType() == 2) A.ApplyToObject(FCKSelection.GetSelectedElement());
			else A.ApplyToSelection(FCK.EditorWindow);
			FCK.Events.FireEvent('OnSelectionChange');
		}
	},
	...
	RemoveAll: function() {
		...
		for (var i = 1; i < G.length; i++) {
			...
			if (E.test(H.nodeName)) FCKDomTools.BreakParent(C, H, A);
		};
		...
		for (var i = 1; i < G.length; i++) {
			...
			if (E.test(H.nodeName)) FCKDomTools.BreakParent(D, H, A);
		};
		var I = FCKDomTools.GetNextSourceNode(C, true, 1);
		while (I) {
			if (I == D) break;
			var J = FCKDomTools.GetNextSourceNode(I, false, 1);
			if (E.test(I.nodeName)) FCKDomTools.RemoveNode(I, true);
			else FCKDomTools.RemoveAttributes(I, FCKConfig.RemoveAttributesArray);
			I = J;
		};
		A.SelectBookmark(B);
		FCK.Events.FireEvent('OnSelectionChange');
	...
	_LoadStylesXml: function() {
		...
		for (var i = 0; i < E.length; i++) {
			...
			Q.Name = F.name || G;
			if (Q.GetType() == 2) this._ObjectStyles[G] = true;
			A[Q.Name] = Q;
		};
		return A;
	}
		...
		if (B.tagName.toUpperCase().Equals('UL', 'OL')) {
			...
			if (L || K.Equals('UL', 'OL')) {
				if (F) {
					...
					D.AppendTo(G);
					A.appendChild(G);
				} else if (L) D.InsertAfterNode(B.parentNode);
				else D.InsertAfterNode(B);
				if (L) FCKDomTools.InsertAfterNode(B.parentNode, B.removeChild(A));
				else FCKDomTools.InsertAfterNode(B, B.removeChild(A));
			} else {
				if (F) {
				...
				if (FCKConfig.EnterMode == 'br') {
					if (FCKBrowserInfo.IsGecko) O.parentNode.insertBefore(FCKTools.CreateBogusBR(C), O);
					else FCKDomTools.InsertAfterNode(O, FCKTools.CreateBogusBR(C));
...
var FCKElementPath = function(A) {
	...
	while (e) {
		if (e.nodeType == 1) {
			...
			if (FCKBrowserInfo.IsIE && e.scopeName != 'HTML') E = e.scopeName.toLowerCase() + ':' + E;
			if (!C) {
				if (!B && FCKListsLib.PathBlockElements[E] != null) B = e;
				if (FCKListsLib.PathBlockLimitElements[E] != null) {
					if (!B && E == 'div' && !FCKElementPath._CheckHasBlock(e)) B = e;
					else C = e;
				}
			};
			D.push(e);
...
FCKElementPath._CheckHasBlock = function(A) {
	...
	for (var i = 0, count = B.length; i < count; i++) {
		var C = B[i];
		if (C.nodeType == 1 && FCKListsLib.BlockElements[C.nodeName.toLowerCase()]) return true;
	};
	return false;
};
...
FCKDomRange.prototype = {
	_UpdateElementInfo: function() {
		...
		if (!A) this.Release(true);
			...
			if (A.collapsed) {
				...
				if (B != D) C = new FCKElementPath(D);
				var E = D;
				if (A.endOffset == 0) {
					while (E && !E.previousSibling) E = E.parentNode;
					if (E) E = E.previousSibling;
				} else if (E.nodeType == 1) E = E.childNodes[A.endOffset - 1];
				this.EndNode = E;
				this.EndContainer = D;
	...
	MoveToElementEditStart: function(A) {
		...
		while (A && A.nodeType == 1) {
			if (FCKDomTools.CheckIsEditable(A)) B = A;
			else if (B) break;
			A = A.firstChild;
		};
		if (B) this.MoveToElementStart(B);
	},
	...
	CheckStartOfBlock: function() {
		...
		if (E > 0) {
			...
			} else F = D.childNodes[E - 1];
		};
		if (!F) F = FCKDomTools.GetPreviousSourceNode(D, true, null, C);
		while (F) {
			switch (F.nodeType) {
				case 1:
					if (!FCKListsLib.InlineChildReqElements[F.nodeName.toLowerCase()]) return A.IsStartOfBlock = false;
					break;
				case 3:
					if (F.nodeValue.Trim().length > 0) return A.IsStartOfBlock = false;
			};
	...
	CheckEndOfBlock: function(A) {
		...
		if (D.nodeType == 3) {
			...
			if (E < G.length) {
				G = G.substr(E);
				if (G.Trim().length != 0) return this._Cache.IsEndOfBlock = false;
			}
		} else F = D.childNodes[E];
		if (!F) F = FCKDomTools.GetNextSourceNode(D, true, null, C);
		var H = false;
		while (F) {
			switch (F.nodeType) {
				case 1:
					var I = F.nodeName.toLowerCase();
					if (FCKListsLib.InlineChildReqElements[I]) break;
					if (I == 'br' && !H) {
						H = true;
						break;
					};
				...
				case 3:
					if (F.nodeValue.Trim().length > 0) return this._Cache.IsEndOfBlock = false;
			};
	...
	CreateBookmark: function(A) {
		...
		if (E) {
			this.SetStart(D, 4);
			this.SetEnd(E, 3);
		} else this.MoveToPosition(D, 4);
		return B;
	},
	GetBookmarkNode: function(A, B) {
		var C = this.Window.document;
		if (B) return A.StartNode || C.getElementById(A.StartId);
		else return A.EndNode || C.getElementById(A.EndId);
	},
	MoveToBookmark: function(A, B) {
		var C = this.GetBookmarkNode(A, true);
		var D = this.GetBookmarkNode(A, false);
		this.SetStart(C, 3);
		if (!B) FCKDomTools.RemoveNode(C);
		if (D) {
			this.SetEnd(D, 3);
	...
	MoveToBookmark2: function(A) {
		...
		while (B.nodeType == 3 && D > B.length) {
			if (!B.nextSibling || B.nextSibling.nodeType != 3) break;
			D -= B.length;
			B = B.nextSibling;
		}
		while (C.nodeType == 3 && E > C.length) {
			if (!C.nextSibling || C.nextSibling.nodeType != 3) break;
			E -= C.length;
			C = C.nextSibling;
		};
	...
	SetStart: function(A, B, C) {
		var D = this._Range;
		if (!D) D = this._Range = this.CreateRange();
		switch (B) {
			case 1:
				D.setStart(A, 0);
				break;
		...
		switch (A) {
			case 'inline_elements':
				...
				if ((B.nodeType == 3 && C >= B.nodeValue.length) || (B.nodeType == 1 && C >= B.childNodes.length) || (B.nodeType != 1 && B.nodeType != 3)) {
					if (B.nodeType != 1) B = B.nextSibling ? null : B.parentNode;
					if (B) {
						while (FCKListsLib.InlineNonEmptyElements[B.nodeName.toLowerCase()]) {
							this._Range.setEndAfter(B);
							if (B != B.parentNode.lastChild) break;
							B = B.parentNode;
						}
					}
				};
			...
			case 'list_contents':
				...
				if (A == 'list_contents' || FCKConfig.EnterMode == 'br') D = FCKListsLib.ListBoundaries;
				if (this.StartBlock && FCKConfig.EnterMode != 'br' && A == 'block_contents') this.SetStart(this.StartBlock, 1);
				else {
					B = this._Range.startContainer;
					if (B.nodeType == 1) {
						var E = B.childNodes[this._Range.startOffset];
						if (E) B = FCKDomTools.GetPreviousSourceNode(E, true);
						else B = B.lastChild || B;
					}
				...
				if (this.EndBlock && FCKConfig.EnterMode != 'br' && A == 'block_contents' && this.EndBlock.nodeName.toLowerCase() != 'li') this.SetEnd(this.EndBlock, 2);
				else {
					B = this._Range.endContainer;
					...
					if (B && B.nodeName.toLowerCase() == 'br') this._Range.setEndAfter(B);
				};
				this._UpdateElementInfo();
		}
	},
	SplitBlock: function(A) {
		...
		if (!this._Range) this.MoveToSelection();
		if (this.StartBlockLimit == this.EndBlockLimit) {
			...
			if (B != 'br') {
				...
				if (!D) D = this.FixBlock(false, B);
			};
			var F = (C != null && this.CheckStartOfBlock());
			var G = (D != null && this.CheckEndOfBlock());
			if (!this.CheckIsEmpty()) this.DeleteContents();
			if (C && D && C == D) {
				if (G) {
					...
					if (FCKBrowserInfo.IsGecko && !C.nodeName.IEquals(['ul', 'ol'])) FCKTools.AppendBogusBr(C);
				}
			};
			return {
				PreviousBlock: C,
	...
	FixBlock: function(A, B) {
		...
		if (FCKDomTools.CheckIsEmptyElement(D, function(element) {
			...
			}) && FCKBrowserInfo.IsGeckoLike) FCKTools.AppendBogusBr(D);
		this.InsertNode(D);
	...
	GetTouchedStartNode: function() {
		var A = this._Range;
		var B = A.startContainer;
		if (A.collapsed || B.nodeType != 1) return B;
...
FCKDomRange.prototype.MoveToSelection = function() {
	...
	if (A.type != 'Control') {
		var B = this._GetSelectionMarkerTag(true);
...
FCKDomRange.prototype.Select = function(A) {
	if (this._Range) this.SelectBookmark(this.CreateBookmark(true), A);
};
FCKDomRange.prototype.SelectBookmark = function(A, B) {
	var C = this.CheckIsCollapsed();
	var D;
	var E;
	var F = this.GetBookmarkNode(A, true);
	if (!F) return;
	var G;
	if (!C) G = this.GetBookmarkNode(A, false);
	var H = this.Window.document.body.createTextRange();
	...
	if (G) {
		...
		D = B || !F.previousSibling || F.previousSibling.nodeName.toLowerCase() == 'br';
...
FCKDomRange.prototype._GetSelectionMarkerTag = function(A) {
	...
	if (D.parentElement().document != B) return null;
	D.collapse(A === true);
...
FCKDomRangeIterator.prototype = {
	GetNextParagraph: function() {
		...
		while (H) {
			...
			if (!K) {
				...
				if (F[M] && (!FCKBrowserInfo.IsIE || H.scopeName == 'HTML')) {
					...
					if (B) {
						B.SetEnd(H, 3, true);
						if (M != 'br') this._NextNode = FCKDomTools.GetNextSourceNode(H, true, null, I) || H;
					};
					J = true;
				} else {
					if (H.firstChild) {
			...
			if (K) B.SetEnd(H, 4, true);
			if ((J || C) && B) {
				B._UpdateElementInfo();
				if (B.StartNode == B.EndNode && B.StartNode.parentNode == B.StartBlockLimit && B.StartNode.getAttribute && B.StartNode.getAttribute('_fck_bookmark')) B = null;
				else break;
			};
			if (C) break;
		...
		if (D) {
			...
			if (P && P.nodeType == 1) {
				if (P.nodeName.toLowerCase() == 'br') P.parentNode.removeChild(P);
				else if (P.lastChild && P.lastChild.nodeName.IEquals('br')) P.removeChild(P.lastChild);
			}
		};
		if (E) {
			var Q = A.lastChild;
			if (Q && Q.nodeType == 1 && Q.nodeName.toLowerCase() == 'br') A.removeChild(Q);
		};
		if (!this._NextNode) this._NextNode = (C || A == I) ? null : FCKDomTools.GetNextSourceNode(A, true, null, I);
		return A;
	}
};
	...
	_ExecContentsAction: function(A, B) {
		...
		if (A == 2) {
			...
			if (topStart && topEnd && (C.parentNode != topStart.parentNode || D.parentNode != topEnd.parentNode)) {
				...
				if (G && topEnd.parentNode == C.parentNode) N--;
...
FCKEnterKey.prototype.DoEnter = function(A, B) {
	...
	if (E == 'br' || D.Block && D.Block.tagName.toLowerCase() == 'pre') return this._ExecuteEnterBr();
	else return this._ExecuteEnterBlock(E);
};
...
FCKEnterKey.prototype._ExecuteBackspace = function(A, B, C) {
	...
	if (B && C) {
		...
		if (FCKListsLib.EmptyElements[G] != null || G == 'table') {
			...
			if (!C.tagName.IEquals(['TABLE'])) FCKDomTools.MoveChildren(C, B);
			A.SelectBookmark(I);
			D = true;
		}
	};
...
FCKEnterKey.prototype.DoTab = function() {
	...
	while (B) {
		if (B.nodeType == 1) {
			...
			if (C == "tr" || C == "td" || C == "th" || C == "tbody" || C == "table") return false;
			else break;
		};
		B = B.parentNode;
	};
...
FCKEnterKey.prototype._ExecuteEnterBlock = function(A, B) {
	...
	if (D) {
		...
		if (!G && !H) {
			if (F.nodeName.IEquals('li') && F.firstChild && F.firstChild.nodeName.IEquals(['ul', 'ol'])) F.insertBefore(FCKTools.GetElementDocument(F).createTextNode('\xa0'), F.firstChild);
			if (F) C.MoveToElementEditStart(F);
		} else {
			...
			} else if (F) I = FCKDomTools.CloneElement(F);
			if (!I) I = this.Window.document.createElement(A);
			var K = D.ElementPath;
			if (K) {
				for (var i = 0, len = K.Elements.length; i < len; i++) {
					var L = K.Elements[i];
					if (L == K.Block || L == K.BlockLimit) break;
			...
			if (FCKBrowserInfo.IsGeckoLike) FCKTools.AppendBogusBr(I);
			C.InsertNode(I);
			if (FCKBrowserInfo.IsIE) {
...
FCKEnterKey.prototype._ExecuteEnterBr = function(A) {
	...
	if (B.StartBlockLimit == B.EndBlockLimit) {
		...
		if (!F && D && (/^H[1-6]$/).test(E)) {
			...
			var H;
			G = E.IEquals('pre');
			if (G) H = this.Window.document.createTextNode(FCKBrowserInfo.IsIE ? '\r' : '\n');
			else H = this.Window.document.createElement('br');
			B.InsertNode(H);
			if (FCKBrowserInfo.IsGecko) FCKDomTools.InsertAfterNode(H, this.Window.document.createTextNode(''));
			if (D && FCKBrowserInfo.IsGeckoLike) FCKTools.AppendBogusBr(H.parentNode);
			if (FCKBrowserInfo.IsIE) B.SetStart(H, 4);
			else B.SetStart(H.nextSibling, 1);
			if (!FCKBrowserInfo.IsIE) {
				var I = null;
				if (FCKBrowserInfo.IsOpera) I = this.Window.document.createElement('span');
				else I = this.Window.document.createElement('br');
...
FCKEnterKey.prototype._CheckIsAllContentsIncluded = function(A, B) {
	var C = false;
	var D = false;
	if (A.StartContainer == B || A.StartContainer == B.firstChild) C = (A._Range.startOffset == 0);
...
FCKDocumentProcessor.Process = function(A) {
	var B = FCK.IsDirty();
	var C, i = 0;
	while ((C = this._Items[i++])) C.ProcessDocument(A);
...
var FCKEmbedAndObjectProcessor = (function() {
	...
	var B = function(el) {
		...
		for (var i = 0; i < A.length; i++) D = A[i](el, D) || D;
		if (D != E) FCKTempBin.RemoveElement(E.getAttribute('_fckrealelement'));
		el.parentNode.replaceChild(D, el);
	};
	var F = function(elementName, doc) {
		var G = doc.getElementsByTagName(elementName);
		for (var i = G.length - 1; i >= 0; i--) B(G[i]);
	};
	var H = function(doc) {
		F('object', doc);
		F('embed', doc);
	...
	return FCKTools.Merge(FCKDocumentProcessor.AppendNew(), {
		ProcessDocument: function(doc) {
			if (FCKBrowserInfo.IsGecko) FCKTools.RunFunction(H, this, [doc]);
			else H(doc);
		},
		RefreshView: function(placeHolder, original) {
			if (original.getAttribute('width') > 0) placeHolder.style.width = FCKTools.ConvertHtmlSizeToStyle(original.getAttribute('width'));
			if (original.getAttribute('height') > 0) placeHolder.style.height = FCKTools.ConvertHtmlSizeToStyle(original.getAttribute('height'));
		},
...
var FCKSelection = FCK.Selection = {
	GetParentBlock: function() {
		...
		while (A) {
			if (FCKListsLib.BlockBoundaries[A.nodeName.toLowerCase()]) break;
			A = A.parentNode;
		};
		return A;
	},
...
FCKSelection.GetType = function() {
	try {
		...
		if (A == 'Control' || A == 'Text') return A;
		if (this.GetSelection().createRange().parentElement) return 'Text';
	} catch (e) {};
	return 'None';
};
FCKSelection.GetSelectedElement = function() {
	if (this.GetType() == 'Control') {
		var A = this.GetSelection().createRange();
		if (A && A.item) return this.GetSelection().createRange().item(0);
	};
	return null;
};
...
FCKSelection.HasAncestorNode = function(A) {
	...
	while (B) {
		if (B.nodeName.IEquals(A)) return true;
		B = B.parentNode;
	};
	return false;
};
FCKSelection.MoveToAncestorNode = function(A) {
	...
	while (B && !B.nodeName.Equals(A)) B = B.parentNode;
	return B;
};
...
FCKSelection.Save = function(A) {
	...
	if (C) {
		...
		if (D) {
			if (D.parentElement && FCKTools.GetElementDocument(D.parentElement()) != B) D = null;
			else if (D.item && FCKTools.GetElementDocument(D.item(0)) != B) D = null;
		}
	};
...
FCKSelection._GetSelectionDocument = function(A) {
	var B = A.createRange();
	if (!B) return null;
	else if (B.item) return FCKTools.GetElementDocument(B.item(0));
	else return FCKTools.GetElementDocument(B.parentElement());
};
...
FCKTableHandler.DeleteRows = function(A) {
	if (!A) {
		...
		for (var i = C.length; i >= 0; i--) {
			if (C[i]) FCKTableHandler.DeleteRows(C[i]);
		};
		return;
	};
...
FCKTableHandler.DeleteTable = function(A) {
	...
	if (A.parentNode.childNodes.length == 1) A.parentNode.parentNode.removeChild(A.parentNode);
	else A.parentNode.removeChild(A);
};
FCKTableHandler.InsertColumn = function(A) {
	var B = null;
	var C = this.GetSelectedCells();
	if (C && C.length) B = C[A ? 0 : (C.length - 1)];
	if (!B) return;
	var D = FCKTools.GetElementAscensor(B, 'TABLE');
	var E = B.cellIndex;
	for (var i = 0; i < D.rows.length; i++) {
		var F = D.rows[i];
		if (F.cells.length < (E + 1)) continue;
		B = F.cells[E].cloneNode(false);
		if (FCKBrowserInfo.IsGeckoLike) FCKTools.AppendBogusBr(B);
		var G = F.cells[E];
...
FCKTableHandler.DeleteColumns = function(A) {
	if (!A) {
		...
		for (var i = B.length; i >= 0; i--) {
			if (B[i]) FCKTableHandler.DeleteColumns(B[i]);
		};
		return;
	};
	if (!A) return;
	...
	for (var i = C.rows.length - 1; i >= 0; i--) {
		...
		if (E.cells[D]) E.removeChild(E.cells[D]);
	}
};
FCKTableHandler.InsertCell = function(A, B) {
	var C = null;
	var D = this.GetSelectedCells();
	if (D && D.length) C = D[B ? 0 : (D.length - 1)];
	if (!C) return null;
	var E = FCK.EditorDocument.createElement('TD');
	if (FCKBrowserInfo.IsGeckoLike) FCKTools.AppendBogusBr(E);
	if (!B && C.cellIndex == C.parentNode.cells.length - 1) C.parentNode.appendChild(E);
	else C.parentNode.insertBefore(E, B ? C : C.nextSibling);
	return E;
};
...
FCKTableHandler._GetMarkerGeometry = function(A, B, C, D) {
	...
	var G = 0;
	var H = 0;
	for (var i = C; A[B][i] && A[B][i][D]; i++) E++;
	for (var i = C - 1; A[B][i] && A[B][i][D]; i--) {
		E++;
		G++;
	};
	for (var i = B; A[i] && A[i][C] && A[i][C][D]; i++) F++;
...
FCKTableHandler.CheckIsSelectionRectangular = function() {
	...
	for (var i = 0; i < A.length; i++) {
		if (A[i].parentNode.parentNode != A[0].parentNode.parentNode) return false;
	};
...
FCKTableHandler.MergeCells = function() {
	...
	for (var i = 0; i < F.height; i++) {
		...
		if (J > 0) I.appendChild(FCK.EditorDocument.createElement('br'));
	};
	...
	B.appendChild(I);
	if (FCKBrowserInfo.IsGeckoLike && (!B.firstChild)) FCKTools.AppendBogusBr(B);
...
FCKTableHandler.MergeRight = function() {
	...
	while (D && D.childNodes && D.childNodes.length > 0) E.appendChild(D.removeChild(D.firstChild));
...
FCKTableHandler.MergeDown = function() {
	...
	while (D && D.childNodes && D.childNodes.length > 0) E.appendChild(D.removeChild(D.firstChild));
	if (E.firstChild) E.insertBefore(FCK.EditorDocument.createElement('br'), E.firstChild);
	B.appendChild(E);
	this._MarkCells([D], '_Replace');
	this._ReplaceCellsByMarker(C, '_Replace', B);
	this._InstallTableMap(C, B.parentNode.parentNode.parentNode);
...
FCKTableHandler.HorizontalSplitCell = function() {
	...
	if (F > 1) {
		...
		if (FCKBrowserInfo.IsGeckoLike) FCKTools.AppendBogusBr(H);
		var I = E + G;
		var J = E + F;
		var K = isNaN(B.rowSpan) ? 1 : B.rowSpan;
		for (var r = D; r < D + K; r++) {
			for (var i = I; i < J; i++) C[r][i] = H;
		}
	} else {
		var L = [];
		for (var i = 0; i < C.length; i++) {
			...
			if (C[i][E] == B) {
				...
				if (FCKBrowserInfo.IsGeckoLike) FCKTools.AppendBogusBr(M[M.length - 1]);
			} else {
				M.push(C[i][E]);
				M.push(C[i][E]);
			};
			for (var j = E + 1; j < C[i].length; j++) M.push(C[i][j]);
			L.push(M);
		};
		C = L;
	};
...
FCKTableHandler.VerticalSplitCell = function() {
	...
	if (G > 1) {
		...
		if (F > 1) K.colSpan = F;
		if (FCKBrowserInfo.IsGeckoLike) FCKTools.AppendBogusBr(K);
		...
		if (N.rows.length > L) N.insertBefore(M, N.rows[L]);
...
FCKTableHandler._InstallTableMap = function(A, B) {
	...
	for (var i = 0; i < A.length; i++) {
		for (var j = 0; j < A[i].length; j++) {
			...
			if (!D) continue;
			if (j > E) E = j;
			if (D._colScanned === true) continue;
			if (A[i][j - 1] == D) D.colSpan++;
			if (A[i][j + 1] != D) D._colScanned = true;
		}
	};
	for (var i = 0; i <= E; i++) {
		for (var j = 0; j < A.length; j++) {
			if (!A[j]) continue;
			var D = A[j][i];
			if (!D || D._rowScanned === true) continue;
			if (A[j - 1] && A[j - 1][i] == D) D[C]++;
			if (!A[j + 1] || A[j + 1][i] != D) D._rowScanned = true;
		}
	};
	for (var i = 0; i < A.length; i++) {
		for (var j = 0; j < A[i].length; j++) {
...
FCKTableHandler.ClearRow = function(A) {
	...
	for (var i = 0; i < B.length; i++) {
		B[i].innerHTML = '';
		if (FCKBrowserInfo.IsGeckoLike) FCKTools.AppendBogusBr(B[i]);
	}
};
FCKTableHandler.GetMergeRightTarget = function() {
	...
	if (H.height != I.height || H.y != I.y) return null;
	return {
		'refCell': B,
		'nextCell': G,
		'tableMap': C
...
FCKTableHandler.GetMergeDownTarget = function() {
	...
	var G = C[F][E];
	if (!G) return null;
	if (B.parentNode.parentNode != G.parentNode.parentNode) return null;
	this._MarkCells([B, G], '_SizeTest');
	var H = this._GetMarkerGeometry(C, D, E, '_SizeTest');
	var I = this._GetMarkerGeometry(C, F, E, '_SizeTest');
	this._UnmarkCells([B, G], '_SizeTest');
	if (H.width != I.width || H.x != I.x) return null;
	return {
		'refCell': B,
		'nextCell': G,
		'tableMap': C
...
FCKXml.prototype = {
	...
	SelectSingleNode: function(A, B) {
		...
		else return this.DOMDocument.selectSingleNode(A);
	}
};
...
FCKStyleCommand.prototype = {
	...
	Execute: function(A, B) {
		FCKUndo.SaveUndoStep();
		if (B.Selected) FCK.Styles.RemoveStyle(B.Style);
	...
	GetState: function() {
		if (FCK.EditMode != 0 || !FCK.EditorDocument) return -1;
		if (FCKSelection.GetType() == 'Control') {
			var A = FCKSelection.GetSelectedElement();
			if (!A || !FCKStyles.CheckHasObjectStyle(A.nodeName.toLowerCase())) return -1;
		};
		return 0;
	}
};
...
FCKSourceCommand.prototype.Execute = function() {
	if (FCKConfig.SourcePopup) {
		...
		FCKDialog.OpenDialog('FCKDialog_Source', FCKLang.Source, 'dialog/fck_source.html', A, B, null, true);
...
FCKVisitLinkCommand.prototype = {
	...
	Execute: function() {
		...
		if (!window.open(B, '_blank')) alert(FCKLang.VisitLinkBlocked);
	}
};
...
FCKPasteCommand.prototype = {
	Execute: function() {
		if (FCKBrowserInfo.IsIE) FCK.Paste();
		else FCK.ExecuteNamedCommand('Paste');
	},
	GetState: function() {
		if (FCK.EditMode != 0) return -1;
...
FCKCutCopyCommand.prototype = {
	Execute: function() {
		...
		if (!A) alert(FCKLang['PasteError' + this.Name]);
	},
	GetState: function() {
...
FCKAnchorDeleteCommand.prototype = {
	Execute: function() {
		if (FCK.Selection.GetType() == 'Control') {
			...
			if (A) {
				if (A.tagName == 'IMG' && A.getAttribute('_fckanchor')) oAnchor = FCK.GetRealElement(A);
				else A = null;
			};
			if (!A) {
				oAnchor = FCK.Selection.MoveToAncestorNode('A');
				if (oAnchor) FCK.Selection.SelectNode(oAnchor);
			};
			if (oAnchor.href.length != 0) {
				oAnchor.removeAttribute('name');
				if (FCKBrowserInfo.IsIE) oAnchor.className = oAnchor.className.replace(FCKRegexLib.FCK_Class, '');
				return;
			};
			if (A) {
				A.parentNode.removeChild(A);
...
FCKDeleteDivCommand.prototype = {
	...
	Execute: function() {
		...
		B.MoveToSelection();
		var C = B.CreateBookmark();
		for (var i = 0; i < A.length; i++) FCKDomTools.RemoveNode(A[i], true);
		B.MoveToBookmark(C);
		B.Select();
	}
};
...
var FCKShowBlockCommand = function(A, B) {
	this.Name = A;
	if (B != undefined) this._SavedState = B;
...
FCKShowBlockCommand.prototype.Execute = function() {
	...
	if (A == 1) B.className = B.className.replace(/(^| )FCK__ShowBlocks/g, '');
	else B.className += ' FCK__ShowBlocks';
	if (FCKBrowserInfo.IsIE) {
		try {
		...
		if (C) {
			if (C.nodeType != 1) C = C.parentNode;
...
FCKShowBlockCommand.prototype.GetState = function() {
	if (FCK.EditMode != 0) return -1;
	if (!FCK.EditorDocument) return 0;
	if (/FCK__ShowBlocks(?:\s|$)/.test(FCK.EditorDocument.body.className)) return 1;
	return 0;
};
...
FCKShowBlockCommand.prototype.RestoreState = function() {
	if (this._SavedState != null && this.GetState() != this._SavedState) this.Execute();
};
...
var FCKTextColorCommand = function(A) {
	...
	else if (FCK.ToolbarSet._IFrame) B = FCKTools.GetElementWindow(FCK.ToolbarSet._IFrame);
	else B = window.parent;
...
FCKTextColorCommand.prototype.SetColor = function(A) {
	...
	if (!A || A.length == 0) FCK.Styles.RemoveStyle(B);
	else {
...
FCKTextColorCommand.prototype._CreatePanelBody = function(A, B) {
	...
	while (H < G.length) {
		...
		for (var i = 0; i < 8; i++, H++) {
			...
			if (H >= G.length) C.style.visibility = 'hidden';
			else FCKTools.AddEventListenerEx(C, 'click', FCKTextColorCommand_OnClick, [this, L]);
		}
	};
...
FCKPasteWordCommand.prototype.GetState = function() {
	if (FCK.EditMode != 0 || FCKConfig.ForcePasteAsPlainText) return -1;
	else return FCK.GetNamedCommandState('Paste');
};
...
FCKTableCommand.prototype.GetState = function() {
	if (FCK.EditorDocument != null && FCKSelection.HasAncestorNode('TABLE')) {
		switch (this.Name) {
			case 'TableHorizontalSplitCell':
			case 'TableVerticalSplitCell':
				if (FCKTableHandler.GetSelectedCells().length == 1) return 0;
				else return -1;
			case 'TableMergeCells':
				if (FCKTableHandler.CheckIsSelectionRectangular() && FCKTableHandler.GetSelectedCells().length > 1) return 0;
				else return -1;
			case 'TableMergeRight':
...
FCKFitWindow.prototype.Execute = function() {
	...
	if (!this.IsMaximized) {
		if (FCKBrowserInfo.IsIE) C.attachEvent('onresize', FCKFitWindow_Resize);
		else C.addEventListener('resize', FCKFitWindow_Resize, true);
		...
		if (FCKBrowserInfo.IsIE) C.detachEvent("onresize", FCKFitWindow_Resize);
		else C.removeEventListener("resize", FCKFitWindow_Resize, true);
		G = A;
		while ((G = G.parentNode)) {
...
FCKFitWindow.prototype.GetState = function() {
	if (FCKConfig.ToolbarLocation != 'In') return -1;
	else return (this.IsMaximized ? 1 : 0);
};
...
FCKListCommand.prototype = {
	GetState: function() {
		...
		if (B && B.nodeName.IEquals(this.TagName)) return 1;
		else return 0;
	},
	Execute: function() {
		FCKUndo.SaveUndoStep();
		...
		while (J) {
			while ((I = H.GetNextParagraph())) {
				...
				if (N) continue;
				var R = O;
				if (R._FCK_ListGroupObject) R._FCK_ListGroupObject.contents.push(I);
				else {
					var Q = {
						'root': R,
						'contents': [I]
			...
			if (FCKBrowserInfo.IsIE) J = false;
			else {
				if (K == null) {
					K = [];
		...
		while (F.length > 0) {
			var Q = F.shift();
			if (C == 0) {
				if (Q.root.nodeName.IEquals(['ul', 'ol'])) this._ChangeListType(Q, G, W);
				else this._CreateList(Q, W);
...
FCKJustifyCommand._GetClassNameRegex = function() {
	...
	if (B.length > 0) A = new RegExp('(?:^|\\s+)(?:' + B.join('|') + ')(?=$|\\s)');
	else A = null;
...
FCKJustifyCommand.prototype = {
	Execute: function() {
		...
		while ((F = E.GetNextParagraph())) {
			...
			if (D) {
				...
				var H = F.style;
				if (B == 0) H.textAlign = this.AlignValue;
				else {
					H.textAlign = '';
	...
	GetState: function() {
		...
		var C;
		if (FCKBrowserInfo.IsIE) C = B.currentStyle.textAlign;
		else C = FCK.EditorWindow.getComputedStyle(B, '').getPropertyValue('text-align');
	...
	_IndentBlock: function(A) {
		...
		while ((E = B.GetNextParagraph())) {
			...
			if (FCKIndentCommand._UseIndentClasses) {
				...
				if (this.Name.IEquals('outdent')) G--;
				else if (this.Name.IEquals('indent')) G++;
				G = Math.min(G, FCKConfig.IndentClasses.length);
				G = Math.max(G, 0);
				var H = E.className.replace(FCKIndentCommand._ClassNameRegex, '');
				if (G < 1) E.className = H;
				else E.className = (H.length > 0 ? H + ' ' : '') + FCKConfig.IndentClasses[G - 1];
			} else {
	...
	Execute: function() {
			...
			if (E && E.parentNode.nodeName.IEquals('blockquote') && !E.previousSibling) {
				...
				while ((F = F.nextSibling)) {
					if (FCKListsLib.BlockElements[F.nodeName.toLowerCase()]) {
						if (F.firstChild == D) FCKDomTools.InsertAfterNode(D, E);
						else FCKDomTools.MoveNode(E, F, true);
					}
				}
			}
		};
		...
		if (A == 0) {
			...
			while (O.length > 0) {
				var W = O.shift();
				var N = W.parentNode;
				if (W == W.parentNode.firstChild) N.parentNode.insertBefore(N.removeChild(W), N);
				else if (W == W.parentNode.lastChild) N.parentNode.insertBefore(N.removeChild(W), N.nextSibling);
				else FCKDomTools.BreakParent(W, W.parentNode, B);
...
FCKCommands.GetCommand = function(A) {
	...
	switch (A) {
		...
		case 'Undefined':
			...
			break;
		case 'Scayt':
			B = FCKScayt.CreateCommand();
			break;
		case 'ScaytContext':
			B = FCKScayt.CreateContextCommand();
			break;
		default:
...
FCKPanel.prototype.Show = function(x, y, A, B, C) {
	...
	if (this._Popup) {
		...
		this._PopupArgs = [x, y, D, E.offsetHeight, A];
...
FCKPanel.prototype.CheckIsOpened = function() {
	if (this._Popup) return this._Popup.isOpen;
	else return this._IsOpened;
};
...
FCKToolbarButtonUI.prototype.Create = function(A) {
	...
	if (this.Style == 0 && !this.ShowArrow) {
		...
		if (this.Style == 0 || this.Style == 2) F.appendChild(this.Icon.CreateIconElement(B));
		else F.appendChild(this._CreatePaddingElement(B));
...
function FCKToolbarButtonUI_OnMouseOver(A, B) {
	if (B.State == 0) this.className = 'TB_Button_Off_Over';
	else if (B.State == 1) this.className = 'TB_Button_On_Over';
};

function FCKToolbarButtonUI_OnMouseOut(A, B) {
	if (B.State == 0) this.className = 'TB_Button_Off';
	else if (B.State == 1) this.className = 'TB_Button_On';
};
...
function FCKSpecialCombo_OnClick(e, A) {
	if (A.Enabled) {
		...
		if (D.offsetHeight > E) C.style.height = E + 'px';
		else C.style.height = '';
...
(function() {
	var A = [];
	var B = (FCK && FCK.EditorWindow && FCK.EditorWindow.parent.parent.scayt) ? true : false;
	var C = false;
	var D = false;
	...
	function ScaytEngineLoad(callback) {
		if (B) return;
		B = true;
		var E = FCK.EditorWindow.parent.parent;
		var F = function() {
			window.scayt = E.scayt;
			InitScayt();
			var G = FCKToolbarItems.LoadedItems['ScaytCombobox'];
			G && G.SetEnabled(scyt_control && scyt_control.disabled);
			InitSetup();
		};
		if (E.scayt) {
			F();
		...
		if (FCK.Config.ScaytCustomUrl) FCK.Config.ScaytCustomUrl = new String(FCK.Config.ScaytCustomUrl).replace(new RegExp("^http[s]*:\/\/"), "");
		var H = document.location.protocol;
		var I = FCK.Config.ScaytCustomUrl || 'svc.spellchecker.net/spellcheck3/lf/scayt/scayt4.js';
		var J = H + '//' + I;
		var K = ParseUrl(J).path + '/';
		var L = E.window.CKEDITOR || (E.window.CKEDITOR = {});
		L._djScaytConfig = {
			I: K,
			addOnLoad: function() {
				F();
	...
	function createScaytControl() {
		var N = {};
		var E = FCK.EditorWindow.parent.parent;
		N.srcNodeRef = FCK.EditingArea.IFrame;
		N.customerid = FCK.Config.ScaytCustomerid;
		N.customDictionaryName = FCK.Config.ScaytCustomDictionaryName;
		N.userDictionaryName = FCK.Config.ScaytUserDictionaryName;
		N.defLang = FCK.Config.ScaytDefLang;
		var P = E.scayt;
		var Q = window.scayt_control = new P(N);
	};

	function InitScayt() {
		createScaytControl();
		var Q = window.scayt_control;
		if (Q) {
			Q.setDisabled(false);
			D = true;
			C = !Q.disabled;
	...
	T.prototype.Execute = function(c) {
		switch (c) {
			case 'Options':
			case 'Langs':
			case 'About':
				...
				if (B && D) FCKDialog.OpenDialog('Scayt', 'SCAYT Settings', 'dialog/fck_scayt.html?' + c.toLowerCase(), 343, 343);
				break;
			default:
				if (!B) {
					var U = this;
					ScaytEngineLoad(function() {
						U.SetEnabled(!window.scayt_control.disabled);
					});
					return true;
				} else if (D) {
					if (C) this.Disable();
		...
		if (!B) return ScaytMessage('SCAYT is not loaded') || false;
		if (!D) return ScaytMessage('SCAYT is not ready') || false;
		return true;
	};
	...
	T.prototype.SetEnabled = function(state) {
	...
	function ShowScaytState() {
		var W = FCKToolbarItems.GetItem('SpellCheck');
		if (!W || !W._Combo || !W._Combo._OuterTable) return;
		var X = W._Combo._OuterTable.getElementsByTagName('img')[1];
		var Y = W._Combo.Items['trigger'];
		if (C) {
			X.style.opacity = '1';
			Y.innerHTML = GetStatusLabel();
		} else {
			X.style.opacity = '0.5';
			Y.innerHTML = GetStatusLabel();
		}
	...
	function GetStatusLabel() {
		if (!D) return '<strong>Enable SCAYT</strong>';
		return C ? '<strong>Disable SCAYT</strong>' : '<strong>Enable SCAYT</strong>';
	};
	var Z = function(tooltip, style) {
	...
	Z.prototype = new FCKToolbarSpecialCombo;
	Z.prototype.CreateItems = function() {
		this._Combo.AddItem('Trigger', '<strong>Enable SCAYT</strong>');
	...
	Z.prototype.GetLabel = function() {
		var a = FCKConfig.SkinPath + 'fck_strip.gif';
		return FCKBrowserInfo.IsIE ? '<div class="TB_Button_Image"><img src="' + a + '" style="top:-192px"></div>' : '<img class="TB_Button_Image" src="' + FCK_SPACER_PATH + '" style="background-position: 0px -192px;background-image: url(' + a + ');">';
	};
	...
	b.prototype.Execute = function(contextInfo) {
		var c = contextInfo && contextInfo.action,
			g = c && contextInfo.node,
			Q = window.scayt_control;
		if (g) {
			switch (c) {
				case 'Suggestion':
					Q.replace(g, contextInfo.suggestion);
					break;
				case 'Ignore':
					Q.ignore(g);
					break;
				...
				case 'Add Word':
					var E = FCK.EditorWindow.parent.parent;
					E.scayt.addWordToUserDictionary(g);
					break;
			}
		}
	};
		...
		FCK.ContextMenu.RegisterListener({
			AddItems: function(menu) {
				...
				var Q = window.scayt_control,
					P = E.scayt;
				if (!Q) return;
				var g = Q.getScaytNode();
				if (!g) return;
				var h = P.getSuggestion(Q.getWord(g), Q.getLang());
				if (!h || !h.length) return;
				menu.AddSeparator();
				var j = FCK.Config.ScaytMaxSuggestions || 5;
				var k = (j == -1) ? h.length : j;
				for (var i = 0; i < k; i += 1) {
					if (h[i]) {
						menu.AddItem('ScaytContext', h[i], null, false, {
							'action': 'Suggestion',
							'node': g,
							'suggestion': h[i]
						});
				...
				menu.AddSeparator();
				menu.AddItem('ScaytContext', 'Ignore', null, false, {
					'action': 'Ignore',
					'node': g
				});
				menu.AddItem('ScaytContext', 'Ignore All', null, false, {
					'action': 'Ignore All',
					'node': g
				});
				menu.AddItem('ScaytContext', 'Add Word', null, false, {
					'action': 'Add Word',
					'node': g
				});
				try {
					if (D && C) Q.fireOnContextMenu(null, FCK.ContextMenu._InnerContextMenu);
				} catch (err) {}
			}
		});
	...
	FCK.Events.AttachEvent('OnAfterSetHTML', function() {
		if (FCKConfig.SpellChecker == 'SCAYT') {
			if (!B && FCK.Config.ScaytAutoStartup) ScaytEngineLoad();
			if (FCK.EditMode == 0 && B && D) createScaytControl();
			ShowScaytState();
		}
	});
	FCK.Events.AttachEvent('OnBeforeGetData', function() {
		D && window.scayt_control.reset();
	});
	FCK.Events.AttachEvent('OnAfterGetData', function() {
		D && window.scayt_control.refresh();
	});
	FCKScayt = {
...
FCKToolbarItems.GetItem = function(A) {
	...
	switch (A) {
		...
		case 'ShowBlocks':
			...
			break;
		case 'SpellCheck':
			if (FCKConfig.SpellChecker == 'SCAYT') B = FCKScayt.CreateToolbarItem();
			else B = new FCKToolbarButton('SpellCheck', FCKLang.SpellCheck, null, null, null, null, 13);
			break;
		default:
...
function FCKToolbarSet_Create(A) {
	...
	switch (C) {
		...
		default:
			...
			var E = C.match(/^Out:(.+)\((\w+)\)$/);
			if (E) {
				if (FCKBrowserInfo.IsAIR) FCKAdobeAIR.ToolbarSet_GetOutElement(window, E);
				else D = eval('parent.' + E[1]).document.getElementById(E[2]);
			} else {
				E = C.match(/^Out:(\w+)$/);
...
var FCKToolbarSet = function(A) {
	...
	if (!FCKConfig.ToolbarCanCollapse || FCKConfig.ToolbarStartExpanded) this.Expand();
	else this.Collapse();
	C.style.display = FCKConfig.ToolbarCanCollapse ? '' : 'none';
	if (FCKConfig.ToolbarCanCollapse) C.style.display = '';
	else A.getElementById('xTBLeftBorder').style.display = '';
	this.Toolbars = [];
	this.IsLoaded = false;
...
FCKToolbarSet.prototype.Load = function(A) {
	...
	if (FCK.Status != 2) FCK.Events.AttachEvent('OnStatusChange', this.RefreshModeState);
	else this.RefreshModeState();
	this.IsLoaded = true;
	this.IsEnabled = true;
	FCKTools.RunFunction(this.OnLoad);
};
...
FCKContextMenu.prototype.AttachToElement = function(A) {
	if (FCKBrowserInfo.IsIE) FCKTools.AddEventListenerEx(A, 'contextmenu', FCKContextMenu_AttachedElement_OnContextMenu, this);
			...
			return {
				AddItems: function(menu, tag, tagName) {
					...
					if (C) {
						...
						if (FCKBrowserInfo.IsGecko) D.AddItem('TableMergeCells', FCKLang.MergeCells, 60, FCKCommands.GetCommand('TableMergeCells').GetState() == -1);
						else {
