...
function userinfoAdd($dbinfo)
{
	...
	$idnum_qs = db_escape($qs['idnum'] ?? ''); // $_POST에서 넘어오지 않을 수 있으므로 널 병합 처리
	...
	if($fieldlist = userGetAppendFields($dbinfo['table_logon'],$skip_fields)) { // 배열 키 수정
		foreach($fieldlist as $value) {
			...
			if(isset($qs[$value])) {
				...
				if($value=='passwd') $sql_set .= ", passwd =password('{$qs_value_escaped}') "; // 이스케이프된 값 사용
				else $sql_set .= ", {$value} = '{$qs_value_escaped}' "; // 이스케이프된 값 사용
			}
			elseif(isset($_POST[$value])) {
				$post_value_escaped = db_escape($_POST[$value]); // 쿼리 삽입 전 이스케이프
				$sql_set .= ", {$value} = '{$post_value_escaped}' "; // 이스케이프된 값 사용
			}
		}
	}
	////////////////////////////////
	...
	$sql="INSERT INTO {$dbinfo['table_logon']}
		...
		"; // table_kmember 대신 table_logon으로 수정 (회원 추가 로직을 따름)
	db_query($sql);
