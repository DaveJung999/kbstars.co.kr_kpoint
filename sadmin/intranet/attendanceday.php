<?php
//=======================================================
// 설 명 : 인트라넷 - 특정일 출근부(attendanceday.php)
...
//=======================================================
	...
	// 넘어온값 필터링
	if(!isset($_GET['searchdate']) || !preg_match("/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/",$_GET['searchdate'])) {
		$_GET['searchdate'] = date("Y-m-d");
	}
	$gname = isset($_GET['gname']) ? $_GET['gname'] : $SITE['th'];

	// 출석체크 그룹 uids 구하기
	$rs_groupinfo	= db_query("select * from {$table_groupinfo} where name='{$gname}'");
	$groupinfo		= db_count() ? db_array($rs_groupinfo) : back("{$gname} - 해당 그룹이 없습니다.");
	$rs_groupuids	= db_query("select * from {$table_groupinfo} where topgid='{$groupinfo['uid']}'");
	$groupuids[]	= $groupinfo['uid'];
	while($rows=db_array($rs_groupuids)) {
		$groupuids[]=$rows['uid'];
	} // end while
	db_free($rs_groupinfo);

	// 그룹내 회원리스트구하기
	$rs_joininfo	= db_query("select distinct bid from {$table_joininfo} where gid in (". implode(",", $groupuids) .") order by bid");
...
//=======================================================
// 해당 월 데이터 변수에 일괄 저장
$rs_attend	= db_query("select * from {$table} where workday='".str_replace("-","",$_GET['searchdate']) . "'");
while($rows=db_array($rs_attend)) {
	$data_attend[$rows['bid']]=$rows;
}
db_free($rs_attend);
?>
<html>
...
<?php
			...
			echo "{$logon['name']} ";// {$userinfo['orders']} {$userinfo['job']}";
			unset($logon);
			unset($userinfo);
...
<?php
				...
				if(!$tmp) echo "-";
				elseif(date("ymd",$tmp)==date("ymd")) echo date("H:i",$tmp);
				else echo date("y-m-d",$tmp);
