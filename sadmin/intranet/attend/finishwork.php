...
$thisUrl = './'; // $thisUrl 초기화
	...
	// 새벽 6시 이전에는 출근 못함
	if(empty($dbinfo['debug']) && date('H')<6) {
		echo "새벽 6시 이전에는 출근 못합니다.";
		exit;
	}
	// 출근 여부 확인
	$seUid_safe = (int)($_SESSION['seUid'] ?? 0);
	$workday_safe = db_escape(date("Ymd"));
	$sql_check = "SELECT * FROM {$dbinfo['table']} WHERE workday='{$workday_safe}' AND bid='{$seUid_safe}'";
	$result_check = db_query($sql_check);
	if($list_tmp = db_array($result_check)) {
		back("이미 {$list_tmp['type']}으로 되어 있습니다.\\n 즐거운 하루 되시기 바랍니다.","./list.php");
	}
	...
	if(isset($list['finishtime']))
		$list['maxworkhours']= (int)( ( $list['finishtime'] - ($list['begintime'] ?? 0) ) / 3600 + 1);
	else
		$list['maxworkhours']= (int)( ( time() - ($list['begintime'] ?? 0) ) / 3600 + 1);
	...
	$form_default .= href_qs("uid={$list['uid']}&mode=finishwork&rdate=".$rdate_get,$qs_basic,1);
	$form_default = substr($form_default,0,-1);

	
	// 6. 공통 할당
	$list['docu_type_checked'] = (isset($list['docu_type']) && strtolower($list['docu_type'] ?? '')=='html') ? ' checked ' : '';
	if( !($mode_get=='modify' && ($list['bid'] ?? '')!=($_SESSION['seUid'] ?? '')) ) {
		...
		$list['email']	= ($_SESSION['seEmail'] ?? '') ? $_SESSION['seEmail'] : ($list['email'] ?? '');
	}
	...
	// TODO
	$list['begintime_date'] = date("Y-m-d H:i:s", $list['begintime'] ?? time());
	if(isset($list['finishtime_date']))
		$list['finishtime_date'] = date("Y-m-d H:i:s", $list['finishtime_date'] ?? time());
	else
		$list['finishtime_date'] = date("Y-m-d H:00:00");
//=======================================================
...
$tpl->set_var('nowdate', date("Y-m-d H시 i분"));
...
// 블럭 : 권한 입력 부분
if(($dbinfo['enable_priv'] ?? '')=='Y')	$tpl->process('PRIV','priv');
