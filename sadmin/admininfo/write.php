...
//=======================================================
	...
	if($_GET['mode'] == 'modify'){
		...
		$list = array (
				'skin' =>	'basic',
				'href_sadminmenu' =>	'/scate/?db=sadmin',
				'href_sadminmain' =>	'/sadmin/admininfo/list.php',
			'html_type' =>	'ht',
			'html_skin' =>	'basic'
			);
		...
		if($dbinfo['default_title']) $list['title'] = $dbinfo['default_title'];
		if($dbinfo['default_content']) $list['content'] = $dbinfo['default_content'];
	}
	
	// 6 . 공통 할당
	$list['docu_type_checked'] = (strtolower($list['docu_type']) == 'html') ? ' checked ' : '';
	if( !($_GET['mode'] == 'modify' and $list['bid'] != $_SESSION['seUid']) ){
