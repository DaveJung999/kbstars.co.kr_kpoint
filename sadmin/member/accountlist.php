...
if(!db_count($rs_accountinfo)) {
	// 생성된 포인트 계좌가 없어서 신규로 생성
	back("생성된 포인트 계좌가 없어서 신규로 생성합니다","/smember/sitebank/firstaccount.php?bid={$bid}&goto=".urlencode($_SERVER['PHP_SELF']."?bid={$bid}"));
}
else {
	while($list=db_array($rs_accountinfo)) {
		...
		if($list['errorno']=0 and $list['balance'] != $deposit-$withdrawal) {
			// 비상 상황 발생!!! 해당 계좌 잔액과 입출금합계 금액이 다름
			$list['errorno']		='1';
			$list['errornotice']	='잔액과 입출금합계 오류 발생';
			db_query("update {$table_accountinfo} set errorno='{$list['errorno']}' , errornotice='{$list['errornotice']}' where bid={$bid} and uid={$list['uid']}");
		...
		$list['account']=preg_replace("/^([0-9]+)([0-9]{5})([0-9][0-9])$/i", "\\1-\\2-\\3", $list['accountno']);
	
		// URL link..
		$href['inquiry']="accountinquiry.php?".href_qs("bid={$bid}&accountno={$list['accountno']}","bid=");
		$href['deposit']="accountdeposit.php?".href_qs("bid={$bid}&accountno={$list['accountno']}","bid=");
		$href['withdrawal']="accountwithdrawal.php?".href_qs("bid={$bid}&accountno={$list['accountno']}","bid=");
		$href['myadmin_modify_accountinfo']="/sadmin/myadmin224/tbl_change.php?table={$table_accountinfo}&primary_key=+%60uid%60+%3D+%27{$list['uid']}%27&goto=" . urlencode($_SERVER["REQUEST_URI"]);
		if($list['transfertype']=="모든이체불가") 
			$href['transfer']="javascript: window.alert(\"본 계좌는 이체나 환불이 되지 않습니다.\\n계좌 종류를 확인 바랍니다.\");";
		else
			$href['transfer']="accounttransfer.php?".href_qs("bid={$bid}&accountno={$list['accountno']}","bid=");
