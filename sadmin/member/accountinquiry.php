...
$form_inquiry = " {$table_account}='{$_SERVER['PHP_SELF']}' method='get'>";
$form_inquiry .= substr(href_qs("mode=inquiry",'',1),0,-1);
$tpl->set_var('form_inquiry',$form_inquiry);

// 기간 조회에 따른 where절 만들기
if($from_year) {
	$from_time	= mktime(0,0,0,$_GET['from_month'], $_GET['from_day'],$_GET['from_year']);
	$to_time	= mktime(23,59,59,$_GET['to_month'], $_GET['to_day']+1,$_GET['to_year']);
}
else {
	$from_time	= mktime(0, 0, 0, date('m')-1, date('d'), date('Y'));
	$to_time	= time();
}
$sql_where = "rdate >= '{$from_time}' and rdate < '{$to_time}' ";

		
$rs_account=db_query("SELECT * from {$table_account} where bid={$bid} and accountno={$_GET['accountno']} and $sql_where order by uid");
unset($before_balance);
	
if(!db_count($rs_account)) {
	...
	while($list=db_array($rs_account)) {
		...
		if(!isset($before_balance)) {
			$before_balance = $list['balance'];
			$list['balance']=number_format($list['balance']);
		}
		else {
			$before_balance += $list['deposit']-$list['withdrawal'];
			if($list['balance']!=$before_balance ) {
				$list['balance'] = "<STRIKE>{$list['balance']}</STRIKE>{$before_balance} ";
				$href['myadmin_modify_account']="/sadmin/myadmin224/tbl_change.php?table={$table_account}&pos=0&session_max_rows=30&disp_direction=horizontal&repeat_cells=100&dontlimitchars=&primary_key=+%60uid%60+%3D+%27{$list['uid']}%27&goto=" . urlencode($_SERVER['PHP_SELF'] . "?" . $_SERVER['QUERY_STRING']);
				$list['balance'] = "<a href='{$href['myadmin_modify_account']}'>{$list['balance']}</a>";
			}
			else $list['balance']=number_format($list['balance']);
		}
	
		// 숫자에 콤모(,) 붙이기
		$list['deposit']=number_format($list['deposit']);
		$list['withdrawal']=number_format($list['withdrawal']);
...
// 템플릿 마무리 할당
$tpl->set_var('from_year'	,date('Y',$from_time));
$tpl->set_var('from_month'	,date('m',$from_time));
$tpl->set_var('from_day'	,date('d',$from_time));
$tpl->set_var('to_year'		,date('Y',$to_time));
$tpl->set_var('to_month'	,date('m',$to_time));
$tpl->set_var('to_day'		,date('d',$to_time));
