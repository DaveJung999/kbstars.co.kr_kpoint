...
if($_REQUEST['mode']=='ok') echo "checked='checked'"; ?>>ok.php
<input type="radio" name="mode" value="sql_post" <?php 
if($_REQUEST['mode']=='sql_post') echo "checked='checked'"; ?>>SQL문($_POST)
<input type="radio" name="mode" value="sql_list" <?php 
if($_REQUEST['mode']=='sql_list') echo "checked='checked'"; ?>>SQL문($list)
...
function ok_php() {
	...
	if(is_array($_REQUEST['fields']) and count($_REQUEST['fields'])) {
		...
		$fields	= userTablelist($GLOBALS['SITE']['database'],$_REQUEST['table']);
		foreach($fields as $key => $value) {
			if($value==$_REQUEST['key']) unset($fields[$key]);
		} // end foreach
	}
...
?>
			WHERE
				{$field}={$qs[$field]} // [!] FIX: WHERE 절 불필요한 따옴표 제거
			AND
				 $sql_where 
		";
	db_query($sql);
