...
/**
  ...
  09/03/03 -- Thanks to Stig Benning for pointing out a javascript
			  ...
			  fixing the problem.
  09/18/03 -- Thanks to Jim Applegate for finding and fixing a bug
			  related to the placement of images.  Sub-choices 
			  were given their parents image for the mouseovers.
			  All images should now be correct.
 ...
 **/
class cascading_verticle_menu {

	var $top;			// Number of pixels from the top edge of the screen
	var $left;			// Number of pixels from the left edge of the screen
	var $height;		 // Height of individual cell in pixels
	var $width;		  // Width of individual cell in pixels
	...
	Constructor
	**/
	function cascading_verticle_menu($t, $l, $h, $w, $c)
	{
	  $this->top = $t;
	  $this->left = $l;
	  $this->height = $h;
	...
	**/
	function write()
	{
	  ...
	  for($i = 0; $i < $n; ++$i)
	  {
		 // ***Corrected***
		 $main_nav .= '<tr>
						<td width="' . $this->choices[$i]->width . 'px" height="' . $this->height . 'px" onmouseover="swap(\'main_nav_' . $i . '_img\', \'' . $this->choices[$i]->down_img . '\');'. $show = (sizeof($this->choices[$i]->choices) ? 'show(\'sub_nav_' . $i . '\');' : '') .'" onmouseout="swap(\'main_nav_' . $i . '_img\', \'' . $this->choices[$i]->up_img . '\');'. $hide = (sizeof($this->choices[$i]->choices) ? 'hide(\'sub_nav_' . $i . '\');' : '') .'" class="' . $this->choices[$i]->css_class . '">';
	...
	**/
	function sub_menu_write($c, $id)
	{
	  ...
	  {
		 // *** Corrected: Thanks to Jim Applegate ***
		 $ret .= ' <tr>
		 ...
		 // Add image and link
		 $ret .= '"><a href="' . $c->choices[$i]->link . '"><img id="' . $id . '_img_' . $i . '" src="' . $c->up_img . '" height="' . $this->height . '" width="' . $c->width . '" border="0" /></a></td>
				  </tr>
				 ';
	  }
	  $ret .= '</table>';
...
class choice {
	...
	var $height;		 // Height of choice in pixels
