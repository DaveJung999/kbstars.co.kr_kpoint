	...
	if($fieldlist = userGetAppendFields($dbinfo['table'], $skip_fields)) {
		foreach($fieldlist as $value) {
			...
			switch($value) {
				// TODO
				case 'startdate' :
					$qs['startdate']=strtotime($_POST['startdate_date'] ?? '');
					break;
				case 'db' :
					if(!preg_match('/^[a-z0-9]+$/i',$_POST['db'] ?? ''))
...
function modify_ok(&$dbinfo,$qs)
{
	...
	// 해당 게시물 읽어오기
	$sql = "SELECT *,password('" . db_escape($_POST['passwd'] ?? '') . "') as pass FROM {$dbinfo['table']} WHERE uid='" . (int)$qs['uid'] . "' and $sql_where LIMIT 1";
	...
	if($fieldlist = userGetAppendFields($dbinfo['table'], $skip_fields)) {
		foreach($fieldlist as $value) {
			...
			switch($value) {
				...
				case 'content' : // <br>테그다음에 꼭 new line 들어가게
					if(isset($qs['content'])) $qs['content'] = preg_replace("/<br>([^\r\n])/i","<br>\n\\1",$qs['content']);
					elseif(isset($_POST['content'])) $qs['content'] = preg_replace("/<br>([^\r\n])/i","<br>\n\\1",$_POST['content']);
					break;				
				case 'docu_type' : // html값이 아니면 text로
					$_POST['docu_type'] = strtolower($_POST['docu_type'] ?? ($dbinfo['default_docu_type'] ?? ''));
					if($_POST['docu_type']!='html') $_POST['docu_type']='text';
...
function delete_ok(&$dbinfo)
{
	...
	// 해당 게시물 읽어오기
	$sql = "SELECT *,password('" . db_escape($qs['passwd'] ?? '') . "') as pass FROM {$dbinfo['table']} WHERE uid='" . (int)$qs['uid'] . "' and $sql_where LIMIT 1";
	...
	// TODO
	userDeleteShop2DB($list['db'] ?? ''); // 해당 게시판 테이블 삭제

	// 삭제
	db_query("DELETE FROM {$dbinfo['table']} WHERE uid='" . (int)$qs['uid'] . "' and  $sql_where ");
	return true;
} // end func delete_ok()
...
function userDeleteShop2DB($db,$forceDelete=0) { // 05/01/24 박선민
	global $db_conn, $SITE;
