...
$dbinfo_from	= array(
	// table
	'table' => 'kbsavers2.'.(isset($_POST['from_table']) ? $_POST['from_table'] : ''),
	'upload_dir' => $_SERVER['DOCUMENT_ROOT'].'/sboard2/upload/'.(isset($_POST['from_table']) ? $_POST['from_table'] : ''),
);

$dbinfo_to	= array(
	// 심플리스트 제목
	'title' => 'kbsavers2.'.(isset($_POST['to_table']) ? $_POST['to_table'] : ''),
	'upload_dir' =>	$_SERVER['DOCUMENT_ROOT'].'/sboard2/upload/'.(isset($_POST['to_table']) ? $_POST['to_table'] : ''),
);

if(isset($_POST['mode']) && $_POST['mode'] == 'ok'){
	global $db_conn;
	...
	if(isset($_POST['from_cateuid']) && $_POST['from_cateuid']){
	...
	if(isset($_POST['to_cateuid']) && $_POST['to_cateuid']){
	...
	$max_num = (int)db_resultone($sql,0,'maxnum');

	$from_cateuid = isset($_POST['from_cateuid']) ? (int)$_POST['from_cateuid'] : 0;
	$sql = "select * from {$dbinfo_from['table']} where cateuid='{$from_cateuid}'";
	$rs = db_query($sql);
	while($from = db_array($rs)){
		...
		@mkdir($updir_to,0777, true);
		
		// 기존 업로드 파일 정보 읽어오기
		if(isset($from['upfiles']) && $from['upfiles']){
			$upfiles=unserialize($from['upfiles']);
			if(!is_array($upfiles)) { // 시리얼화된 변수가 아닌 파일 명으로 되어 있을 경우
				$upfiles['upfile']['name'] = isset($from['upfiles']) ? $from['upfiles'] : '';
				$upfiles['upfile']['size'] = isset($from['upfiles_totalsize']) ? (int)$from['upfiles_totalsize'] : 0;
			}
		}
		$upfiles_totalsize=(int)$from['upfiles_totalsize'];

		// 대상 파일이 이미 존재하면 파일명 변경
		if(isset($upfiles) and is_array($upfiles) and count($upfiles)>0){
		...
		";
		if(isset($_POST['test']) && $_POST['test'])
			echo "<br><div style='overflow:scroll; height:70; width:500;'>".htmlspecialchars($sql) . "</div><br>";
		else
			db_query($sql);
	}
}
...
<input type="hidden" name="mode" value="ok">
	<p>원본 테이블
	<input name="from_table" type="text" id="from_table" value="<?php echo isset($_POST['from_table']) ? $_POST['from_table'] : ''; ?>">
	<br>
	원본 카테고리
	<input name="from_cateuid" type="text" id="from_cateuid"value="<?php echo isset($_POST['from_cateuid']) ? $_POST['from_cateuid'] : ''; ?>">
	</p>
	<p>대상	테이블
	<input name="to_table" type="text" id="to_table"value="<?php echo isset($_POST['to_table']) ? $_POST['to_table'] : ''; ?>">
	<br>
대상 카테고리
<input name="to_cateuid" type="text" id="to_cateuid"value="<?php echo isset($_POST['to_cateuid']) ? $_POST['to_cateuid'] : ''; ?>">
</p>
<p><input name="test" type="checkbox" value="1" <?php echo (isset($_POST['test']) && $_POST['test'] == '1' ? 'checked="checked"' : ''); ?>>
	테스트</p>
