...
//=======================================================
	...
	if ($_GET['mode'] == 'allreg' ){
		...
		$sql = "select * 
				 ...
				 where pid={$_GET['pid']} LIMIT 1"; // SQL 내 변수 사용 중괄호 처리
...
function	userEnumSetFieldsToOptionTag($table,&$list) { // 05/02/03 박선민
	$table_def = db_query('SHOW FIELDS FROM '.$table); // mysql_query() -> db_query()
	if(!$table_def) return;
...
function	userGetDefaultFromTable($table,$field='') { // 05/02/03 박선민
	...
	// @mysql_query('SHOW COLUMNS FROM `'.$table.'` '.$sql_like) -> db_query()
	$table_def = db_query('SHOW COLUMNS FROM `'.db_escape($table).'` '.$sql_like); // db_escape() 적용
	if(!$table_def) return;
