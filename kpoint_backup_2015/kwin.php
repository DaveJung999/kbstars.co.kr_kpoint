...
//=======================================================
	...
	if (isset($_GET['mode']) && $_GET['mode'] == 'windelete' ){
		...
		// 중괄호 사용 시 변수명 명확히
		$form_default .= substr(href_qs("mode=winpoint_delete&s_id={$s_id}&cur_sid={$cur_sid}&type={$kpoint['type']}",'mode=',1),0,-1);
	}else if (isset($_GET['mode']) && $_GET['mode'] == 'win' ){
		// 템플릿 마무리 할당
		...
		$form_default = " method='post' action='kok.php'>";
		// 중괄호 사용 시 변수명 명확히
		$form_default .= substr(href_qs("mode=winpoint_add&s_id={$s_id}&cur_sid={$cur_sid}",'mode=',1),0,-1);
	}
