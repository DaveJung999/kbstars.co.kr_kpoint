...
//=======================================================
	...
	$table_present		= $SITE['th'] . "kpoint_present";
...
// 회원 정보 가져옴
$sql_where_member = " mid = {$_GET['mid']} ";
$sql_member = "SELECT * FROM $table_kmember	WHERE $sql_where_member ";
$list_member=db_arrayone($sql_member);

$href['season']="kseason.php?accountno=$list_member['accountno']&mid=$list_member['mid']&s_id={$_GET['s_id']}&cur_sid={$_GET['cur_sid']}&name=$list_member['name']";
$href['p_write']="kpresent.php?mode=p_write&mid=$list_member['mid']&s_id={$_GET['s_id']}&cur_sid={$_GET['cur_sid']}&point=$list_member['balance']";
...
$sql_kpointinfo = "SELECT * FROM $table_kpointinfo WHERE bid={$_GET['mid']} ORDER BY s_id desc";
...
if ($total > 0) {
	while($list=db_array($rs_kpointinfo)) {
		...
		// URL link..
		$href['inquiry']="{$_SERVER['PHP_SELF']}?mode=inquiry&accountno={$list['accountno']}&mid={$list['bid']}&s_id={$list['s_id']}&cur_sid={$_GET['cur_sid']}";
		$href['s_modify']="kseason.php?mode=s_modify&accountno={$list['accountno']}&uid={$list['uid']}&mid={$list['bid']}&s_id={$list['s_id']}&cur_sid={$_GET['cur_sid']}&name={$list['name']}";
		$href['s_delete']="kok.php?mode=s_delete&accountno={$list['accountno']}&uid={$list['uid']}&mid={$_GET['mid']}&s_id={$list['s_id']}&cur_sid={$_GET['cur_sid']}";
...
if(is_array($kpointinfo)) {
	...
	// 경품 지급
	$sql_present = "select * from $table_present where bid={$_GET['mid']} and s_id = {$_GET['s_id']}  order by pdate desc";
	...
	if(!$total){
		...
		while($kpresent_list=db_array($rs_present)) {
			...
			$href['p_modify']="kpresent.php?mode=p_modify&uid=$kpresent_list['uid']&mid=$kpresent_list['bid']&s_id=$kpresent_list['s_id']&cur_sid={$_GET['cur_sid']}";
			$href['p_delete']="kok.php?mode=p_delete&uid=$kpresent_list['uid']&mid=$kpresent_list['bid']&s_id=$kpresent_list['s_id']&cur_sid={$_GET['cur_sid']}";
	...
	// 포인트 적립 내역
	$sql_account = "select * from $table_kpoint where bid={$_GET['mid']} and s_id = {$_GET['s_id']}  order by rdate desc";
	...
	if(!$total){
		...
		while($kpoint_list=db_array($rs_account)) {
			...
			$href['pl_modify']="kpoint.php?mode=modify&accountno=$kpoint_list['accountno']&pid=$kpoint_list['pid']&mid={$_GET['mid']}&s_id=$kpoint_list['s_id']&cur_sid={$_GET['cur_sid']}";
			$href['pl_delete']="kok.php?mode=kpointdelete&accountno=$kpoint_list['accountno']&pid=$kpoint_list['pid']&mid={$_GET['mid']}&s_id=$kpoint_list['s_id']&cur_sid={$_GET['cur_sid']}";
...
/*	$from_year = $from_year ? $from_year : date(Y);
	...
	$href['write']="kpoint.php?mode=write&accountno=$kpointinfo['accountno']&mid=$kpointinfo['bid']&s_id={$kpointinfo['s_id']}&cur_sid={$_GET['cur_sid']}";
	...
	$href['kread_excel']="kread_excel.php?accountno=$kpointinfo['accountno']&mid=$kpointinfo['bid']&s_id={$kpointinfo['s_id']}&cur_sid={$_GET['cur_sid']}&name={$kpointinfo['name']}";
