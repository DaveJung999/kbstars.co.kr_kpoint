...
//=======================================================
	...
	$table_kpresent = $SITE['th'] . 'kpoint_present';
	
	$dbinfo = array(
				'skin'	=> 'basic',
				'table'	=> $table_kpresent				
			);
			
	
	if ($_GET['mode'] == 'p_modify' ){
		$sql = "select * from $table_kpresent WHERE uid={$_GET['uid']} LIMIT 1";
		...
		$form_default .= substr(href_qs("mode=p_modify&uid={$kpresent_list['uid']}&mid={$_GET['mid']}&s_id={$_GET['s_id']}&cur_sid={$_GET['cur_sid']}",'mode=',1),0,-1);
	}else{		
		// 템플릿 마무리 할당
		$kpresent_list = userGetDefaultFromTable($dbinfo['table']);
		
		if ($_GET['point']) $kpresent_list['point'] = $_GET['point'];
		if (!$kpresent_list['pdate']) $kpresent_list['pdate'] = date("Y-m-d");
		$kpresent_list['present'] = "구단양말";
		
		userEnumSetFieldsToOptionTag($dbinfo['table'],$kpresent_list); // $list['필드_option']에 enum,set필드 <option>..</option>생성
	
		$form_default = " method='post' action='kok.php'>";
		$form_default .= substr(href_qs("mode=p_write&mid={$_GET['mid']}&s_id={$_GET['s_id']}&cur_sid={$_GET['cur_sid']}",'mode=',1),0,-1);
	}
