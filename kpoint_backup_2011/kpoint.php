...
//=======================================================
	...
	if($cnt)	{
		for($i = 0 ; $i < $cnt ; $i++)	{
			...
			if ($i == 0 && !isset($_GET['s_id'])) { // isset 체크로 변경
				$_GET['s_id'] = $list['s_id'];
				$kpoint['s_name'] = $list['s_name']; // kpoint 배열에 저장
			}
	...
	if (isset($_GET['mode']) && $_GET['mode'] == 'modify' ){ // isset 체크 추가
		...
		$sql = "select * from {$table_kpoint} where pid={$_GET['pid']} LIMIT 1";
		$kpoint = db_arrayone($sql) or back('해당 데이터가 없습니다.');
		
		$form_default = " method='post' action='kok.php'>";
		// href_qs 내 배열 변수 접근 시 중괄호 사용
		$form_default .= substr(href_qs("mode=kpointmodify&pid={$kpoint['pid']}&mid={$_GET['mid']}&accountno={$kpoint['accountno']}&cur_sid={$_GET['cur_sid']}",'mode=',1),0,-1);
	}else{
		// 템플릿 마무리 할당
		$kpoint = userGetDefaultFromTable($dbinfo['table']);
		
		if (isset($_GET['accountno'])) $kpoint['accountno'] = $_GET['accountno']; // isset 체크 추가
		...
		$form_default = " method='post' action='kok.php'>";
		// href_qs 내 배열 변수 접근 시 중괄호 사용
		$form_default .= substr(href_qs("mode=kpointadd&mid={$_GET['mid']}&accountno={$kpoint['accountno']}&cur_sid={$_GET['cur_sid']}",'mode=',1),0,-1);
		$kpoint['type'] = "홈경기(주중)";
		$kpoint['deposit'] = "200";
	}
