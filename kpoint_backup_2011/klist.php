...
//=======================================================
	...
	if ($_GET['cur_sid']) $_GET['s_id'] = $_GET['cur_sid'];
	...
	if(trim($act)) {
		if(trim($sdate) AND trim($edate)) {
			...
			if($ssdate > $eedate) back("기간이 잘못설정했습니다");		
				$sql_where .= " and (A.rdate >= $ssdate) AND (A.rdate <= $eedate ) ";
		}
	...
	// 페이지 나눔등 각종 카운트 구하기
	$count['total']=db_resultone("SELECT count(*) FROM $table_kmember as A LEFT JOIN $table_kpointinfo as B ON B.bid=A.mid WHERE $sql_where LIMIT 1", 0, 'count(*)'); // 전체 게시물 수
	$count=board2Count($count['total'],$_GET['page'],$dbinfo['pern'],$dbinfo['page_pern']); // 각종 카운트 구하기
	$count['today']=db_resultone("SELECT count(*) FROM $table_kmember as A LEFT JOIN $table_kpointinfo as B ON B.bid=A.mid	WHERE (A.rdate > unix_timestamp(curdate())) and $sql_where LIMIT 1", 0, 'count(*)');
...
//=======================================================
	...
	if($_GET['sorderby']) {
		...
		if ($_GET['sorderby'] == 'balance') $sql_orderby = "B.".$sql_orderby . " desc, name ";
	}
...
$sql = "SELECT A.*, B.balance FROM $table_kmember as A LEFT JOIN $table_kpointinfo as B ON B.bid=A.mid WHERE  $sql_where ORDER BY $sql_orderby  LIMIT $limitno,$limitrows";
...
while($list=db_array($rs_member)) {
	...
	$href['inquiry'] = "kread.php?". href_qs('mode=inquiry&mid='.$list['mid']."&s_id=".$_GET['s_id']."&cur_sid=".$_GET['s_id']."&accountno=".$list['accountno'],$qs_basic);
	$href['member'] = "kmember.php?". href_qs('mode=modify&mid='.$list['mid']."&s_id=".$_GET['s_id']."&cur_sid=".$_GET['s_id']."&accountno=".$list['accountno'],$qs_basic);
...
$href['klist_excel']="klist_excel.php?mode=win&s_id=$_GET['s_id']";
...
$tpl->set_var('href.klist_excel'			,$href['klist_excel']);	// 게시판 각종 링크
