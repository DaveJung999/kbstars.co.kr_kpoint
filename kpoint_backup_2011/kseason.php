...
//=======================================================
	...
	if($cnt)	{
		for($i = 0 ; $i < $cnt ; $i++)	{
			$list = db_array($rs);
			//최신 시즌
			if ($i == 0 && !isset($_GET['s_id'])) { // isset 체크로 변경
				$_GET['s_id'] = $list['s_id'];
				$s_name = $list['s_name'];
			}
			
			if(isset($_GET['s_id']) && $_GET['s_id'] == $list['s_id']){ // isset 체크로 변경
				$sselect .= "<option value={$list['s_id']} selected>{$list['s_name']}</option>";
				$s_name = $list['s_name'];
			}else
				$sselect .= "<option value={$list['s_id']}>{$list['s_name']}</option>";
		}		
		db_free($rs); // 결과 셋 해제 추가
	}	
	
	if ($_GET['mode'] == 's_modify' ){
		// SQL 내 배열 변수 접근 시 중괄호 사용
		$sql = "select * from {$table_kpointinfo} WHERE uid={$_GET['uid']} LIMIT 1";
		$kpointinfo = db_arrayone($sql) or back('해당 데이터가 없습니다.');
		
		$form_default = " method='post' action='kok.php'>";
		// href_qs 내 배열 변수 접근 시 중괄호 사용
		$form_default .= substr(href_qs("mode=s_modify&uid={$kpointinfo['uid']}&mid={$_GET['mid']}&s_name={$s_name}&cur_sid={$_GET['cur_sid']}",'mode=',1),0,-1);
	}else{		
		// 템플릿 마무리 할당
		$kpointinfo = userGetDefaultFromTable($dbinfo['table']);
		
		if (isset($_GET['accountno'])) $kpointinfo['accountno'] = $_GET['accountno']; // isset 체크 추가
		if (isset($_GET['name'])) $kpointinfo['name'] = $_GET['name']; // isset 체크 추가
		
		userEnumSetFieldsToOptionTag($dbinfo['table'],$kpointinfo); // $list['필드_option']에 enum,set필드 <option>..</option>생성
	
		$form_default = " method='post' action='kok.php'>";
		// href_qs 내 배열 변수 접근 시 중괄호 사용
		$form_default .= substr(href_qs("mode=kseason&mid={$_GET['mid']}&s_name={$s_name}&cur_sid={$_GET['cur_sid']}",'mode=',1),0,-1);
	}
