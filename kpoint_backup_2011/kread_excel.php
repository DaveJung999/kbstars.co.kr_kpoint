...
$sql_kpointinfo = "SELECT * FROM $table_kpointinfo WHERE bid={$_GET['mid']} ORDER BY uid";
...
if ($total > 0) {
	while($list=db_array($rs_kpointinfo)) {
		...
		// URL link..
		$href['inquiry']="{$_SERVER['PHP_SELF']}?mode=inquiry&accountno={$list['accountno']}&mid={$list['bid']}&s_id={$list['s_id']}";
		$href['season']="kseason.php?accountno={$list['accountno']}&mid={$list['bid']}&s_id={$list['s_id']}&name={$list['name']}";
		$href['s_modify']="kseason.php?mode=s_modify&accountno={$list['accountno']}&uid={$list['uid']}&mid={$list['bid']}&s_id={$list['s_id']}&name={$list['name']}";
		$href['s_delete']="kok.php?mode=s_delete&accountno={$list['accountno']}&uid={$list['uid']}&mid={$_GET['mid']}&s_id={$list['s_id']}";
	...
	if(is_array($kpointinfo)) {
		...
		if(!$total){
			...
			while($kpresent_list=db_array($rs_present)) {
				...
				$href['p_modify']="kpresent.php?mode=p_modify&uid=$kpresent_list['uid']&mid=$kpresent_list['bid']&s_id=$kpresent_list['s_id']";
				$href['p_delete']="kok.php?mode=p_delete&uid=$kpresent_list['uid']&bid=$kpresent_list['bid']&s_id=$kpresent_list['s_id']";
		...
		$href['p_write']="kpresent.php?mode=p_write&mid=$kpointinfo['bid']&s_id={$kpointinfo['s_id']}&point=$kpointinfo['balance']";
		
		// 포인트 적립 내역
		$sql_account = "select * from $table_kpoint where bid={$_GET['mid']} and accountno={$kpointinfo['accountno']} and s_id = {$_GET['s_id']}  order by rdate desc";
		...
		if(!$total){
			...
			while($kpoint_list=db_array($rs_account)) {
				...
				$href['modify']="kpoint.php?mode=modify&accountno=$kpoint_list['accountno']&pid=$kpoint_list['pid']&mid={$_GET['mid']}&s_id=$kpoint_list['s_id']";
				$href['delete']="kok.php?mode=kpointdelete&accountno=$kpoint_list['accountno']&pid=$kpoint_list['pid']&mid={$_GET['mid']}&s_id=$kpoint_list['s_id']";
		...
		$href['write']="kpoint.php?mode=write&accountno=$kpointinfo['accountno']&mid=$kpointinfo['bid']&s_id={$kpointinfo['s_id']}";
