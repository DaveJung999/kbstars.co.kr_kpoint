<?php
// 04/05/04 박선민 class_pcrypt.php 함수에서 include()함.
// +----------------------------------------------------------------------+
...
class pcrypt_blowfish
{
	 ...
	 * @return void
	 */
	private function _blowfish_crypt(&$Xl, &$Xr)
	{
		for($i = 0; $i < 16; $i++)
		{
			$Xl = $Xl ^ $this->bctx['p'][$i];
	 ...
	 * @return void
	 */
	private function _blowfish_decrypt(&$Xl, &$Xr)
	{
		for($i = 17; $i >= 2 ; $i--)
		{
			$Xl = $Xl ^ $this->bctx['p'][$i];
	...
	private function _F($x)
	{
		...
		// The additions are performed modulo 2^32. On 64-bit PHP, standard integers
		// are large enough that we don't overflow to float, so we can perform the
		// operations and then truncate the result to 32 bits using & 0xFFFFFFFF.
		$y = ($this->bctx['sb'][0][$a] + $this->bctx['sb'][1][$b]) & 0xFFFFFFFF;
		$y = $y ^ $this->bctx['sb'][2][$c];
		$y = ($y + $this->bctx['sb'][3][$d]) & 0xFFFFFFFF;
