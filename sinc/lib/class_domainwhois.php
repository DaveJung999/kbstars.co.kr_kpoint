...
class domain {
	...
	function info(){
		if($this->is_valid()){
			...
			if($whois_server != ""){
				...
				// Checking whois server for .com and .net and .edu
				if($tldname == "com" || $tldname == "net" || $tldname == "edu"){
					...
					// Getting whois information from secondary whois server
					$fp = @fsockopen($whois_server,43);
					if(!$fp){
						return "Could not connect to whois server.";
					}
	...
	*/
	function html_info(){
		return nl2br(htmlspecialchars($this->info(), ENT_QUOTES | ENT_HTML5));
	}
	...
	*/
	function get_tld(){
		...
		if ($partsCount > 2){
			return implode(".", array_slice($domainParts, 1));
		} elseif($partsCount == 2){
	...
	* @return string $notfound	the string which will be returned by the whois server of the tld if a domain is available
	* @desc Returns the string which will be returned by the whois server of the tld if a domain is available
	*/
	...
	*/
	function is_available(){
		...
		if(empty($whois_string) || empty($not_found_string)){
			return false;
		}

		// Remove domain name from whois string to avoid false positives
		$whois_string2 = str_ireplace($domain, "", $whois_string);

		$whois_string = preg_replace("/\s+/", " ", $whois_string2); //Replace whitespace with single space

		$array = explode(":", $not_found_string);

		if($array['0'] == "MAXCHARS"){
			if(strlen($whois_string) <= intval($array['1'])){
				return true;
			} else {
				return false;
			}
	...
	* @return boolean $is_valid Returns true if domain is valid and false if domain isn't valid
	* @desc Returns if the domain name is valid
	*/
	function is_valid(){
		$domainArr = explode(".", $this->domain);

		// If it's a tld with two parts (like co.uk)
		...
		if(preg_match($pattern, $this->get_domain()) && !preg_match("/--/", $this->get_domain())){
			return true;
		} else {
			return false;
		}
