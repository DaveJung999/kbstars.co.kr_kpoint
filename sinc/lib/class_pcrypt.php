<?php
// 04/05/04 박선민 http://www.phpclasses.org/browse/package/1610.html 에서
/* 사용법 :
require 'class_pcrypt.php';
	
//MODE: MODE_ECB or MODE_CBC
//ALGO: BLOWFISH
//KEY:	Your secret key :) (max lenght: 56)
$crypt = new pcrypt(MODE_ECB, "BLOWFISH", "secretkey"); // 초기화
...
class pcrypt
{
	...
	function _ecb_encrypt($plain)
	{
		...
		for($i = 0;$i < $plainsize;$i = $i + $blocksize)
		{
			...
			{
				// pad block with '\0'
				$block = str_pad($block,$blocksize,"\0",STR_PAD_RIGHT);
			}
	...
	function _cbc_encrypt($plain)
	{
		...
		for($i = 0;$i < $plainsize;$i = $i + $blocksize)
		{
			...
			$xored = $this->_xor_strings($block, $lcipher);
			$lcipher = $this->cipher->_encrypt($xored);
			$cipher .= $lcipher;
		}
	...
	function _cbc_decrypt($cipher)
	{
		...
		for($i = 0;$i < $ciphersize;$i = $i + $blocksize)
		{
			...
			$decrypted	= $this->cipher->_decrypt($block);
			$dblock = $this->_xor_strings($lcipher, $decrypted);
			$lcipher = $block;
			
			 // Remove padded chars 
			$dblock = rtrim($dblock, "\0");
	...
	/** XOR 두 문자열을 계산하는 헬퍼 함수 (길이는 짧은 쪽 기준)
		*
		* @access private
		* @param string $str1
		* @param string $str2
		* @return string
		*/
	function _xor_strings($str1, $str2)
	{
		$result = '';
