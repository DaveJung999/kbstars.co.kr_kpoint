<?php
//---------------------------------------------------------------------------//
// author: pukomuko <salna@ktl.mii.lt>
// date:	2001.03.15
// web:	 http://pukomuko.esu.lt
// info:	template engine
//---------------------------------------------------------------------------//
// copyleft license
//
// this software is provided 'as-is', without any express or implied
// warranty. in no event will the authors be held liable for any damages
// arising from the use of this software.
//
// permission is granted to anyone to use this software for any purpose,
// including commercial applications, and to alter it and redistribute it
// freely, subject to the following restrictions:
//
// 1. the origin of this software must not be misrepresented;
//		you must not claim that you wrote the original software.
//		if you use this software in a product, an acknowledgment
//		in the product documentation would be appreciated but is not required.
//
// 2. altered source versions must be plainly marked as such,
//		and must not be misrepresented as being the original software.
//
// 3. mail about the fact of using this class in production
//		would be very appreciated.
//
// 4. this notice may not be removed or altered from any source distribution.
//
//---------------------------------------------------------------------------//
// changes:
//
//	2004.04.21
//		- warining on empty file
//		* v1.10.1
//
//	2004.03.04
//		+ TPL_LOOPOPT -> TPL_OPTLOOP
//		+ TPL_PARSEDLOOP -> TPL_PARSEDLOOP
//		* v1.10
//
//	2004.03.03
//		+ TPL_LOOP_INNER_PARSED
//		+ TPL_LOOP_INNER_OPTIONAL
//		* v1.10beta2
//
//	2004.03.02
//		+ TPL_STRIP_UTF_HEADER
//		+ TPL_PARSEDLOOP
//		+ TPL_LOOPOPT
//		* v1.10beta
//		+ utf header matching
//
//	2004.03.01
//		- process() bugfix
//		* v1.9.4
//
//	2003.10.20
//		- parse() bugfix
//		* v1.9.3
//
//	2003.08.12
//		- remove_nonjs bugfix
//		* v1.9.2
//
//	2003.06.29
//		* optional now works with 0 and '0'
//		* v1.9.1
//
//	2003.06.19
//		+ custom block syntax, thanks to G. van den Hoven
//		+ set_block_syntax()
//		+ tie_loop()
//		+ tie_var()
//		* v1.9
//
//	2003.06.17
//		* parse() works faster, thanks to Sergej Kurakin
//		- remove_nonjs bugfix
//		+ error handler support
//
//	2003.06.16
//		- set_root() bug
//
//	2003.03.14
//		+ process() now accepts parameters as bits, thanks to Audrius Karabanovas
//		+ set_params() set default parameters for process()
//		* v1.8.1
//		
//	2003.03.14
//		+ phpdoc comments
//		* v1.8
//
//	2002.11.13
//		- $block_names no more
//		- bug in extract blocks
//
//	2002.09.24
//		* parse now accpets string instead of handle
//		+ optional() <opt>
//		* root change
//
//	2002.03.23
//		- bug with templates caintaining { a { b }
//		* v1.7.1
//
//	2002.03.03
//		* fopen fread fclose instead of implode(file()), up to 3x faster
//		+ remove_nonjs - remove only variables that have no spaces in them
//		* speed improvements, got rid of list() = each()
//		+ license :]
//		* v1.7
//
//	2002.03.02
//		- deleted space before each line in the parsed template.
//		- error in method error() :]
//		* v1.6.2 note released :]
//
//	2001.12.06
//		- bug then text had only }
//		* v1.6.1
//
//	2001.10.31
//		+ error on unclosed block
//		+ nested blocks
//		* v1.6
//		
//	2001.09.17
//		- fixed bug with 'keep'
//
//	2001.09.02
//		+ get_var_silent()
//
//	2001.08.09
//		+ error handler
//
//	2001.07.04
//		+ one pass substitution
//
//	2001.05.28
//		+ block name recording
//		* v1.5.1
//
//	2001.05.10
//		- bug in documentation
//
//	2001.05.08
//		* first public release v1.5
//
//	2001.04.04
//		* changed blocks setup, constructor parameters change
//		- error bug
//		- some warnings

/**
*/
define('TPL_LOOP',		1);
define('TPL_NOLOOP',	2);
define('TPL_INCLUDE',	4);
define('TPL_APPEND',	8);
define('TPL_FINISH',	16);
define('TPL_OPTIONAL',	32);
define('TPL_LOOP_INNER_PARSED',64);
define('TPL_LOOP_INNER_OPTIONAL',	128);

define('TPL_PARSEDLOOP', TPL_LOOP | TPL_LOOP_INNER_PARSED);
define('TPL_OPTLOOP',	TPL_PARSEDLOOP | TPL_LOOP_INNER_OPTIONAL);


define('TPL_BLOCK',	 1);
define('TPL_BLOCKREC',	2);
define('TPL_STRIP_UTF_HEADER',	4);

class phemplate
{
	/**
	* variables and blocks
	*/
	public $vars = [];

	/**
	* loops container
	*/
	public $loops = [];

	/**
	* dir of template files
	*/
	public $root = '';

	/**
	* what to do with unknown variables in template?
	*/
	public $unknowns = 'keep';

	/**
	* default parameters for process() loop, append, finish
	*/
	public $parameters = 0;
	

	/**
	* object having method report($level, $msg)
	* you can catch error messages from template.
	*/
	public $error_handler = null;
	

	
	/**
	* start tag for block
	*/
	public $block_start_string = '<block name="|">';

	/**
	* end tag for block
	*/
	public $block_end_string = '</block name="|">';

	/**
	* constructor
	* $unknows를 keep에서 remove_nonjs로 변경함 by Sunmin Park
	*/
	public function __construct( $root_dir = '', $unknowns = 'remove_nonjs', $params = 0)
	{
		$this->set_root($root_dir);
		$this->set_unknowns($unknowns);
		$this->set_params($params);
	}

	/**
	* check and set template root dir
	*/
	public function set_root($root)
	{
		if (empty($root)) return;
		if (!is_dir($root))
		{
			$this->error("phemplate::set_root(): {$root} is not a directory.", 'warning');
			return false;
		}
		
		$this->root = realpath($root).'/'; // By Sunmin 05/11/13
		return true;
	}

	/**
	* what to do with unknown variables in template?
	* keep
	* remove
	* remove_nonjs
	* comment
	* space
	*/
	public function set_unknowns($unk)
	{
		$this->unknowns = $unk;
	}

	/**
	* set default parameters for process()
	*/
	public function set_params($params)
	{
		$this->parameters = $params;
	}

	/**
	* read file template into $vars
	* @param string $handle - name of handle for file contents
	* @param int $blocks - 1 for <block> search, 2 for nested blocks
	*
	*/
	public function set_file($handle, $filename = "", $blocks = false)
	{
		if ($filename == "")
		{
			$this->error("phemplate::set_file(): filename for handle '{$handle}' is empty.", 'fatal');
			return false;
		}
		
		$file_content = $this->read_file($filename);
		if ($file_content === false){
			return false;
		}
		$this->vars[$handle] = $file_content;

		if ($blocks & TPL_STRIP_UTF_HEADER)
		{
		$HEADER = substr($this->vars[$handle], 0, 3);
			if ("\xEF\xBB\xBF" == $header) $this->vars[$handle] = substr($this->vars[$handle], 3);
		}
		if ($blocks) { $this->extract_blocks($handle, $blocks & TPL_BLOCKREC); }
		return true;

	}


	/**
	* set handle and value,
	* if value is array, all elements will be named: handle.key.subkey
	*/
	public function set_var($var_name, $var_value)
	{
		if (is_array($var_value))
		{
			foreach($var_value as $key => $value)
			{
				$this->set_var($var_name . '.' . $key, $value); // recursion for array branches
			}
		}
		else // normal variable
		{
			$this->vars[$var_name] = $var_value;
		}
	}

	/**
	* tie value with variable
	* it's not possible to update array keys, just values
	*/
	public function tie_var($var_name, &$var_value)
	{
		if (is_array($var_value))
		{
			$list = array_keys($var_value);
			foreach($list as $key)
			{
				$this->tie_var($var_name . '.' . $key, $var_value[$key]); // recursion for array branches
			}
		}
		else // normal variable
		{
			$this->vars[$var_name] =& $var_value;
		}
	}

	/**
	* value of $handle
	* raises error if $handle undefined
	*/
	public function get_var($handle)
	{
		if (!isset($this->vars[$handle])){
			return null; // By Sunmin Park 04/05/17
		}
		return $this->vars[$handle];
	}

	/**
	* value of $handle, no error if no handle
	*/
	public function get_var_silent($handle)
	{
		return $this->vars[$handle] ?? '';
	}

	/**
	* assign array to loop handle
	* @param string $loop_name - name for a loop
	* @param int $loop - loop data
	*/
	public function set_loop($loop_name, $loop)
	{
		if (!$loop) $loop = [];
		$this->loops[$loop_name] = $loop;
	}

	/**
	* tie array with loop
	*/
	public function tie_loop($loop_name, &$loop)
	{
		if (!$loop) $loop = [];
		$this->loops[$loop_name] =& $loop;
	}

	/**
	* extracts blocks from handle, and returns cleaned up version
	*/
	public function extract_blocks($bl_handle, $recurse = false)
	{
		$str = $this->get_var($bl_handle);
		if (!$str) return $str;
		$bl_start = 0;

		[$bll, $blr] = explode('|', $this->block_start_string);
		$strlen = strlen($bll);


		// find them and clean from parent handle
		while(is_long($bl_start = strpos($str, $bll, $bl_start)))
		{
			$pos = $bl_start + $strlen;

			$endpos = strpos($str, $blr, $pos);
			$handle = substr($str, $pos, $endpos-$pos);

			$tag = $bll.$handle.$blr;
			$endtag = str_replace('|', $handle, $this->block_end_string);

			$start_pos = $bl_start + strlen($tag);
			$end_pos = strpos($str, $endtag, $bl_start);
			if (!$end_pos) { $this->error("phemplate(): block '{$handle}' has no ending tag", 'fatal'); }
			$bl_end = $end_pos + strlen($endtag);

			$block_code = substr($str, $start_pos, $end_pos-$start_pos);
			
			$this->set_var($handle, $block_code);

			$part1 = substr($str, 0, $bl_start);
			$part2 = substr($str, $bl_end, strlen($str));
			
			$str = $part1 . $part2;

			if ($recurse) { $this->extract_blocks($handle, 1); }

		}

		$this->set_var($bl_handle, $str);
	}


	/**
	* search for <include> tags in $handle
	*/
	public function include_files($handle)
	{
		$str = $this->get_var_silent($handle);

		while(is_long($pos = strpos($str, '<include filename="')))
		{
			$pos += 19;
			$endpos = strpos($str, '">', $pos);
			$filename = substr($str, $pos, $endpos-$pos);
			$tag = '<include filename="'.$filename.'">';

			
			$include = $this->read_file($filename);
			if ($include === false){
				$include = '';
			}

			$str = str_replace($tag, $include, $str);
		}

		return $str;
	}


	/**
	* searches for all set loops in $handle, returns text of $handle with parsed loops
	*/
	public function parse_loops($handle, $noloop = false)
	{
		$str = $this->get_var_silent($handle);
		
		foreach ($this->loops as $loop_name => $loop_ar)
		{
			$start_tag = strpos($str, '<loop name="'.$loop_name.'">');
			if ($start_tag === false) continue;

			$start_pos = $start_tag + strlen('<loop name="'.$loop_name.'">');
			$end_pos = strpos($str, '</loop name="'.$loop_name.'">');
			if ($end_pos === false) continue; // 닫는 태그가 없으면 건너뛰기

			$loop_code = substr($str, $start_pos, $end_pos-$start_pos);
			$org_loop_code = $loop_code;
			
			$start_tag_full = substr($str, $start_tag, ($start_pos - $start_tag));
			$end_tag_full = '</loop name="'.$loop_name.'">';

			if($loop_code !== ''){
					
					$new_code = '';
					$noloop_code = '';

					// clean <noloop... statement from loopcode
					if ($noloop & TPL_NOLOOP)
					{
						$nl_start_tag = strpos($loop_code, '<noloop name="'.$loop_name.'">');
						if ($nl_start_tag !== false){
							$nl_start_pos = $nl_start_tag + strlen('<noloop name="'.$loop_name.'">');
							$nl_end_pos = strpos($loop_code, '</noloop name="'.$loop_name.'">', $nl_start_pos);
	
							if ($nl_end_pos !== false) {
								$noloop_code = substr($loop_code, $nl_start_pos, $nl_end_pos - $nl_start_pos);
								$nl_tag_full = substr($loop_code, $nl_start_tag, ($nl_end_pos + strlen('</noloop name="'.$loop_name.'">')) - $nl_start_tag);
								$loop_code = str_replace($nl_tag_full, '', $loop_code);
							}
						}
					}

					if (is_array($loop_ar) && count($loop_ar) > 0)
					{
						if ($noloop & TPL_LOOP_INNER_PARSED)
						{
							foreach($loop_ar as $row)
							{
								$temp_code = $loop_code;
								// rememeber loop variables

								$array_keys = array_keys($row);
								$this->set_var($loop_name, $row);
								if ($noloop & TPL_LOOP_INNER_OPTIONAL) $temp_code = $this->optional($temp_code);
								$temp_code = $this->parse($temp_code); // parse()는 이제 addslashes를 하지 않음
								$new_code .= $temp_code;

								// cleanup loop variables for next generation
								foreach ($array_keys as $key) unset($this->vars[$loop_name.'.'.$key]);

							} // for loop
						}
						else
						{
							// repeat for every row in array
							foreach($loop_ar as $row)
							{
								$temp_code = $loop_code;

								foreach( $row as $k => $v)
								{
									// 여기서 addslashes를 제거하여 중복 이스케이프 방지
									$temp_code = str_replace( '{'. $loop_name. '.' .$k. '}', $v, $temp_code);
								}
								$new_code .= $temp_code;
							}
						}
					} elseif($noloop & TPL_NOLOOP)
					{
							$new_code = $noloop_code;
					}


					$str = str_replace($start_tag_full . $org_loop_code . $end_tag_full, $new_code, $str);
			} // if loop code
			
		} // repeat for loop names
	
		return $str;
	}
	

	/**
	 * substitute text and returns parsed string
	 * Reverted to return raw values, preventing double-escaping issues.
	 */
	public function parse($string)
	{
		return preg_replace_callback(
			'/\{([a-zA-Z0-9_.]+)\}/',
			function ($matches) {
				$key = $matches['1'];

				if (isset($this->vars[$key])) {
					// addslashes 제거! 원본 값을 그대로 반환합니다.
					return $this->vars[$key];
				}

				switch ($this->unknowns) {
					case 'remove':
					case 'remove_nonjs':
						return '';
					case 'comment':
						return '';
					case 'space':
						return '&nbsp;';
					case 'keep':
					default:
						return $matches['0'];
				}
			},
			$string
		);
	}


	/**
	* process left variables
	* @deprecated parse() now handles unknown variables
	*/
	public function finish($str)
	{
		switch ($this->unknowns)
		{
			case "keep":
			break;

			case "remove":
			$str = preg_replace('/{[^ \t\r\n}]+}/', "", $str);
			break;

			case "comment":
			$str = preg_replace('/{([^ \t\r\n}]+)}/', "", $str);
			break;
			
			case "space":
			$str = preg_replace('/{([^ \t\r\n}]+)}/', "&nbsp;", $str);
			break;

		}

		return $str;
	}

	/**
	* search for optional tag
	*/
	public function optional($str)
	{
		$bl_start = 0;

		// extract and clean them from parent handle
		while(is_long($bl_start = strpos($str, '<opt name="', $bl_start)))
		{
			$pos = $bl_start + 11;

			$endpos = strpos($str, '">', $pos);
			if ($endpos === false) break;

			$varname = substr($str, $pos, $endpos-$pos);
			
			$tag = '<opt name="'.$varname.'">';
			$endtag = '</opt name="'.$varname.'">';

			$end_pos = strpos($str, $endtag, $bl_start);
			if (!$end_pos) { $this->error("phemplate(): optional '{$varname}' has no ending tag", 'fatal'); }

			$bl_end = $end_pos + strlen($endtag);


			$part1 = substr($str, 0, $bl_start);
			$part2 = substr($str, $bl_end, strlen($str));
			
			$flag = false;
			$value = null;

			// if <opt name="!varname">.. by Sunmin Park
			if($varname['0'] == "!")
			{
				$varname = substr($varname, 1);
				$value = $this->get_var_silent($varname);
				if (empty($value) && $value !== 0 && $value !== '0'){
					$flag = true;
				}
			} else {
				$value = $this->get_var_silent($varname);
				if (!empty($value) || $value === 0 || $value === '0'){
					$flag = true;
				}
			}

			if($flag)
			{
				$start_pos = $bl_start + strlen($tag);

				$block_code = substr($str, $start_pos, $end_pos-$start_pos);
				
				$str = $part1 . $this->parse($block_code) . $part2;
			}
			else
			{
				$str = $part1 . $part2;
			}
			
			$bl_start = strlen($part1);
		}

		return $str;
	}

	/**
	* does everything: loops, includes, variables, concatenation
	*/
	public function process($target, $handle, $loop_params = 0)
	{
		if ($loop_params === 0) $loop_params = $this->parameters;
		
		$include_flag = $loop_params & TPL_INCLUDE;
		$append_flag = $loop_params & TPL_APPEND;
		$finish_flag = $loop_params & TPL_FINISH;
		$optional_flag = $loop_params & TPL_OPTIONAL;
		
		$app = ($append_flag && isset($this->vars[$target])) ? $this->get_var($target) : '';

		$content = $this->get_var($handle);

		if ($include_flag) { $content = $this->include_files($content); }

		// Note: parse_loops now operates on a variable, not a handle
		// This requires a temporary variable setup if you want to keep state
		$this->set_var('__temp_process', $content);
		$content = $this->parse_loops('__temp_process', $loop_params);
		$this->drop_var('__temp_process');

		if ($optional_flag) { $content = $this->optional($content); }

		$content = $this->parse($content);

		if ($finish_flag) { $content = $this->finish($content); }
		
		$this->set_var($target, $app . $content);
		
		return $this->get_var($target);
	}


	/**
	* spits out file contents
	* @access private
	*/
	public function read_file($file)
	{
		$filename = $this->root . $file;

		if (!file_exists($filename))
		{
			$this->error("phemplate::read_file(): file {$filename} does not exist.", 'fatal');
			return false;
		}

		$tmp = false;
		
		$filesize = filesize($filename);
		if ($filesize)
		{
			$fp = @fopen($filename, 'r');
			if ($fp === false){
				$this->error("phemplate::read_file(): failed to open file {$filename}.", 'fatal');
				return false;
			}
			$tmp = fread($fp, $filesize);
			fclose($fp);
		}
		return $tmp;
	}

	/** * free mem used by loop
	*/
	public function drop_loop($loop_handle)
	{
		if (isset($this->loops[$loop_handle])) unset($this->loops[$loop_handle]);
	}


	/** * free mem used by var
	*/
	public function drop_var($handle, $var_value = '')
	{
		if (is_array($var_value)) { // by Sunmin Park
			foreach ($var_value as $key => $value){
				$this->drop_var($handle.'.'.$key, $value); // recursion for array branches
			}
		} elseif (isset($this->vars[$handle])){
			unset($this->vars[$handle]);
		}
	}

	/**
	* set error handler
	*/
	public function set_error_handler(&$eh)
	{
		$this->error_handler =& $eh;
	}

	/**
	* change block syntax
	*/
	public function set_block_syntax($start, $end)
	{
		if (strpos($start, '|') === false) $this->error("phemplate::set_block_syntax(): no '|' in start tag", 'fatal');
		if (strpos($end, '|') === false) $this->error("phemplate::set_block_syntax(): no '|' in end tag", 'fatal');
		$this->block_start_string = $start;
		$this->block_end_string = $end;
	}
	
	/**
	* error report
	*/
	public function error( $msg, $level = '')
	{
		if (isset($this->error_handler))
		{
			$lvl = E_USER_WARNING;
			if ('fatal' == $level) $lvl = E_USER_ERROR;
			$this->error_handler->report($lvl, $msg);
		}
		else
		{
			echo "\n<br><font color='#CC0099'><b>{$level}:</b> $msg</font><br>\n";
			if ('fatal' == $level) { exit; }
		}
	}
	
	/**
	* SitePHPbasic 반복하는 echoHtml
	* By Sunmin Park 05/11/20
	*/
	public function echoHtml( &$dbinfo, &$SITE, $thisUrl=''){
		$val = '\\1'.$thisUrl.'skin/'.($dbinfo['skin'] ?? 'basic').'/images/';
		$html_type = $dbinfo['html_type'] ?? 'default'; // Notice 오류 방지

		// - 사이트 템플릿 읽어오기
		if(preg_match('/^(ht|h|t)$/', $html_type)){
		$HEADER['html_echo'] = 2; // This assignment has no effect here
			$html_skin = $dbinfo['html_skin'] ?? 'basic';
			if( is_file($SITE['html_path'].'index_'.$html_skin.'.php') )
				@include($SITE['html_path'].'index_'.$html_skin.'.php');
			else
				@include($SITE['html_path'].'index_basic.php');
		}
		
		$processed_html = $this->process('html', 'html', TPL_OPTIONAL);

		$head = $SITE['head'] ?? '';
		$tail = $SITE['tail'] ?? '';
		$html_head = $dbinfo['html_head'] ?? '';
		$html_tail = $dbinfo['html_tail'] ?? '';

		switch($html_type){
			case 'ht':
				echo $head . $html_head;
				echo preg_replace('/([="\'(])images\//',$val,$processed_html);
				echo $html_tail . $tail;
				break;
			case 'h':
				echo $head . $html_head;
				echo preg_replace('/([="\'(])images\//',$val,$processed_html);
				echo $html_tail;
				break;
			case 't':
				echo $html_head;
				echo preg_replace('/([="\'(])images\//',$val,$processed_html);
				echo $html_tail . $tail;
				break;
			case 'no':
				echo preg_replace('/([="\'(])images\//',$val,$processed_html);
				break;
			default:
				echo $html_head;
				echo preg_replace('/([="\'(])images\//',$val,$processed_html);
				echo $html_tail;
		} // end switch
	}
}
?>