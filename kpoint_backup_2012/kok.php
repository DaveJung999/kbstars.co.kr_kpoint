	...
	case 'kpoint':
		...
		if(isset($_GET['csn']) && $_GET['csn']) {
			$sql = "select * 
					...
					where accountno=".db_escape($_GET['csn']);
...
/*		if($_GET['goto']) $goto = $_GET['goto'];
	...
	default :
		...
		if( isset($_GET['mode']) && preg_match('/^[a-z0-9\-\_]+$/i', $_GET['mode'])
			&& function_exists('mode_'.$_GET['mode']) ) {
...
function kpoint_ok($dbinfo, $klogon)
{
	...
	if( isset($_GET['csn']) && $_GET['csn']) {
		...
		$sql_game = "select * 
					  ...
					  where from_unixtime(g_start,'%Y-%m-%d') =".db_escape($rdate_date)." 
						and (g_home = '13' or g_away = '13') ";
		...
		$sql_season = "select * 
						...
						where sid = ".db_escape($list_game['sid']);
		...
		$sql = "select pid 
				 ...
				 where accountno=".db_escape($_GET['csn'])." 
					and rdate_date=".db_escape($rdate_date)." 
					and remark=".db_escape($remark);
...
function kseason_ok($dbinfo)
{
	...
	if(!db_count($rs_accountinfo)) {
		...
		$sql = "INSERT INTO {$dbinfo['table_kpointinfo']} (`bid`,`accountno`, `userid`, `name`, `s_id`, `s_name`, `accounttype`, `transfertype`,  `balance`, `comment`, `errorno`, `errornotice`, `rdate`) 
					VALUES (".$qs['bid'].", ".$qs['accountno'].",".$qs['userid'].", ".$qs['name'].", ".$list_season['s_id'].", ".$list_season['s_name'].", ".$list_season['s_name']." 적립포인트', '', '$tankyoupoint', '', '0', '', UNIX_TIMESTAMP())";
		db_query($sql);
...
function memSeasonReg_ok($dbinfo)
{
	...
	$sql = "select *
			...
			where (A.priv like '%서포터즈%' or A.priv like '%운영자%' or A.priv like '%포인트관리자%')
			  and A.accountno != ''
			  and not exists(
				SELECT 'X' 
	...
	if(!$total=db_count($rs_accountinfo)) { // 회원별 입력할 정보가 없을때
		...
		for($i = 0 ; $i < $total ; $i++)	{
			...
			$sql_ins = "INSERT INTO {$dbinfo['table_kpointinfo']} (`bid`,`accountno`, `userid`, `name`, `s_id`, `s_name`, `accounttype`, `transfertype`,  `balance`, `comment`, `errorno`, `errornotice`, `rdate`) 
						VALUES (".$list['uid'].", ".$list['accountno'].",".$list['userid'].", ".$list['name'].", ".$qs['s_id'].", ".$qs['s_name'].", ".$qs['s_name']." 적립포인트', '', '$tankyoupoint', '', '0', '', UNIX_TIMESTAMP())";
			db_query($sql_ins);
...
function p_write_ok($dbinfo)
{
	...
	//경품지급 등록
	$rs_kpresent=db_query("SELECT * FROM {$dbinfo['table_kpresent']} WHERE bid = {$qs['bid']} and present=".$qs['present']." and s_id = {$qs['s_id']} LIMIT 1");
	if(!$kpresent = db_count($rs_kpresent)) {
		$sql = "INSERT INTO {$dbinfo['table_kpresent']} (`bid`, `s_id`, `point`, `pdate`, `present`,  `memo`, `rdate`) 
				VALUES (".$qs['bid'].", ".$qs['s_id'].",".$qs['point'].", '$pdate', ".$qs['present'].", ".$qs['memo'].",  UNIX_TIMESTAMP())";
		db_query($sql);
		if(!$accountno = db_insert_id()) {
...
function winpoint_add_ok($dbinfo)
{
	...
	$sql = "SELECT * 
			FROM {$dbinfo['table_kpoint']} 
			WHERE rdate_date=".$qs['rdate_date']." 
			and (type='홈경기(주중)' or type='홈경기(주말)' or type='어웨이경기' )";
	$rs_kpoint = db_query($sql) ;
	$total = db_count($rs_kpoint);
	if(!$total){
...
function winpoint_delete_ok(&$dbinfo)
{
	...
	$sql = "SELECT * 
			FROM {$dbinfo['table_kpoint']} 
			WHERE rdate_date=".$qs['rdate_date']." and type=".$qs['type'];
	$rs_kpoint = db_query($sql) ;
	$total = db_count($rs_kpoint);
	if(!$total){
