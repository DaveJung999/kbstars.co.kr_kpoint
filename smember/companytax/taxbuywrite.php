<?php
//=======================================================
// 설	명 : 매입세금계산서 신규/수정 - PHP 7+ 사용자 DB 함수 적용
...
//=======================================================
	...
	global $SITE; // $mysqli 대신 사용자 정의 DB 함수를 사용하므로 제거
	...
	if($mode === "modify"){
		...
		// SQL Injection 방지를 위해 db_escape() 사용 (정수형 데이터여도 안전을 위해)
		$safe_uid = db_escape((string)$uid);
		
		// Prepared statement 대신 db_query() 및 db_array() 사용
		$sql = "select * from {$dbinfo['table']} where uid='{$safe_uid}'";
		$result = db_query($sql);
		
		// db_array()는 첫 번째 레코드를 가져오고, 더 이상 레코드가 없으면 false를 반환한다고 가정
		$list = db_array($result);
		db_free($result); // 결과 집합 메모리 해제
...
function	userEnumFieldsToOptionTag(string $table, array &$list){
	// mysqli 대신 사용자 정의 db_escape() 사용
	$safe_table = db_escape($table);
	
	// $mysqli->query() 대신 db_query() 사용
	$table_def = db_query("SHOW FIELDS FROM {$safe_table}");
	if(!$table_def) return;
	
	// $table_def->fetch_assoc() 대신 db_array() 사용
	...
	db_free($table_def); // $table_def->free() 대신 db_free() 사용
} // end function
?>